...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .trip-card-list { display: flex; flex-direction: column; gap: 10px; }
  .trip-card { background: var(--surface); border: 1px solid var(--border); border-radius: 12px; padding: 13px 15px; cursor: pointer; }
  .trip-card:active { background: var(--surface-2); }
  .trip-card-top { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 10px; padding-bottom: 8px; border-bottom: 1px dashed var(--border); }
  .trip-card-id { font-family: 'Oswald', sans-serif; font-weight: 700; font-size: 13.5px; color: var(--text); }
  .trip-card-truck { font-family: 'IBM Plex Mono', monospace; font-weight: 500; color: var(--text-dim); font-size: 12px; }
...
function ExpensesList({ trips, computeTripProfit, computeTripFinancials, openTrip, openNewTrip, tripsMonth, setTripsMonth, tripsYear, setTripsYear, settings }) {
  ...
  const totalDispatchFees = rows.reduce((s, r) => s + (r.fin.mode === "fee" ? r.fin.dispatchFee : 0), 0);
  ...
  return (
    <div>
      ...
      <div className="monthly-summary-box">
        ...
        <div className="pay-summary-row"><span>Total Dispatch Fees</span><span style={{ color: "var(--green)" }}>{money(totalDispatchFees)}</span></div>
