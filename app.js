...
const emptyLoad = () => ({
  ...
  invoiceStage: "none", invoicedAt: null, paidAt: null,
});
function normalizeLoad(load) {
  if (load.stops && load.stops.length) {
    return { paidStatus: "unpaid", paidStubId: null, bolDataUri: "", bolFileName: "", bolType: "", invoiceStage: "none", invoicedAt: null, paidAt: null, dispatcher: "", shipperWarehouseCode: "", ...load };
  }
...
const emptyBillTo = () => ({ id: null, name: "", contact: "", phone: "", email: "", address: "", paymentTerms: "" });
...
function normalizeTrip(t) {
  const base = { paidStatus: "unpaid", paidStubId: null, driver2Color: "clear", otherChargesList: [], ...t };
...
function DispatchApp({ onSignOut, userEmail } = {}) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const [t, tr, dr, bt, sh, rc, tp, hi, st, ifr, ifra, iff, dsp] = results;
        ...
        const settingsDefaults = { startingLoadNumber: 1000, theme: "dark", companyName: "", companyAddress: "", dotNumber: "", companyEmail: "", dispatchFeeSchedule: [], dispatcherPaySchedule: [], companyLogoDataUri: "" };
  ...
  function runDashUpdate() {
    setDashApplied({
      ...
      drivers: dashDriverFilter, truck: dashTruckFilter, billTo: dashBillToFilter, dispatcher: dashDispatcherFilter,
    });
  }
  ...
  function computeDriverPay(driverName, start, end) {
    ...
    return { driver, loadPays, grossPay, isMileage, loadedMilesPay, emptyMilesPay, dispatchFeePercent, dispatchFee, expenseBreakdown, expenseTotal, refunds, netPay, trucksUsed, tripIds, otherChargeItems };
  }

  const dashReport = useMemo(() => {
    const f = dashApplied || { start: dashStart, end: dashEnd, viewBy: dashViewBy, drivers: dashDriverFilter, truck: dashTruckFilter, billTo: dashBillToFilter, dispatcher: dashDispatcherFilter };
    ...
    if (f.viewBy === "dispatcher") {
      ...
      const matchingLoads = selectedName ? loads.filter((l) => l.status === "completed" && l.dispatcher === selectedName && inRange(l.deliveryDate || l.pickupDate, f.start, f.end)) : [];
    ...
    return { viewBy: f.viewBy, rows, totals, start: f.start, end: f.end };
  }, [loads, drivers, trips, dashApplied, dashStart, dashEnd, dashViewBy, dashDriverFilter, dashTruckFilter, dashBillToFilter, dashDispatcherFilter, dispatcherByName, settings]);
      ...
      <div className="app-shell">
        ...
        <div className={`content ${textScale === "large" ? "text-large" : ""}`}>
          ...
          {tab === "fleet" && fleetView === "shippers" && (
            <ShippersPage {...{ setFleetView, shipperForm, setShipperForm, saveShipper, editingShipperId, setEditingShipperId, filteredShippers, editShipper, removeShipper, removeShippers, removeAllShippers, fleetSearch, setFleetSearch, startImport, askConfirm }} />
          )}
          {tab === "fleet" && fleetView === "receivers" && (
            <ReceiversPage {...{ setFleetView, receiverForm, setReceiverForm, saveReceiver, editingReceiverId, setEditingReceiverId, filteredReceivers, editReceiver, removeReceiver, removeReceivers, removeAllReceivers, fleetSearch, setFleetSearch, startImport, askConfirm }} />
          )}
          ...
          {tab === "fleet" && fleetView === "accounting" && (
            <AccountingPage {...{
              ...
              askConfirm, companyInfo: settings, setTab, editLoad, deleteLoad,
            }} />
          )}
        </div>
...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .trip-fee-gross-group { display: flex; gap: 10px; }
  ...
  .rate-field-input { border: 2px solid var(--accent) !important; }
        ...
        {rows.map(({ trip: t, miles, gross, fin }) => (
          <div className="trip-card" key={t.id} onClick={() => openTrip(t)}>
            <div className="trip-card-top">
              <span className="trip-card-id"><span className="trip-number-pill">Trip {t.tripNumber}</span> <span className="trip-card-truck truck-standout">· Truck {t.truck}</span></span>
              <span className="trip-card-meta trip-card-meta-bold trip-date-lg">{shortDate(t.startDate)} – {shortDate(t.endDate)}</span>
            </div>
            <div className="dash-card-body trip-stats-row">
              <div className="dash-card-hero trip-miles-figure">
                <span className="lbl">Miles</span>
                <span className="val trip-miles-val">{miles.toLocaleString()}</span>
              </div>
              <div className="trip-fee-gross-group">
                <div className="dash-card-hero figure-pill">
                ...
                <div className="dash-card-hero figure-pill"><span className="lbl">Gross</span><span className="val" style={{ color: "var(--accent)" }}>{compactMoney(gross)}</span></div>
              </div>
            </div>
          </div>
        ))}
        ...
        {(() => {
          ...
          return (
            <>
              ...
              <div className="field-row">{renderField(byKey.orPermit)}{renderField(byKey.logbook)}</div>
              <div className="field-row">{renderField(byKey.insurance)}{renderField(byKey.truckPay)}</div>
            </>
          );
        })()}
            ...
            <table>
              <tbody>
                ...
                {DRIVER_DEDUCTION_FIELDS.filter((f) => f.key !== "otherCharges").map((f) => (r.expenseBreakdown && r.expenseBreakdown[f.key] > 0) ? (
                  <tr className="fee-row" key={f.key}><td>{f.label}</td><td style={{ textAlign: "right" }}>-{money(r.expenseBreakdown[f.key])}</td></tr>
                ) : null)}
                  ...
                  <table>
                    <tbody>
                      ...
                      {DRIVER_DEDUCTION_FIELDS.filter((f) => f.key !== "otherCharges").map((f) => stub.expenseBreakdown[f.key] > 0 ? (
                        <tr className="fee-row" key={f.key}><td>{f.label}</td><td style={{ textAlign: "right" }}>-{money(stub.expenseBreakdown[f.key])}</td></tr>
                      ) : null)}
...
function ShippersPage(p) {
  const { setFleetView, shipperForm, setShipperForm, saveShipper, editingShipperId, setEditingShipperId, filteredShippers, editShipper, removeShipper, removeShippers, removeAllShippers, fleetSearch, setFleetSearch, startImport, askConfirm } = p;
...
function ReceiversPage(p) {
  const { setFleetView, receiverForm, setReceiverForm, saveReceiver, editingReceiverId, setEditingReceiverId, filteredReceivers, editReceiver, removeReceiver, removeReceivers, removeAllReceivers, fleetSearch, setFleetSearch, startImport, askConfirm } = p;
  const [selected, setSelected] = useState({});
  const selectedIds = Object.keys(selected).filter((id) => selected[id]);
...
function AccountingPage(p) {
  const { setFleetView, loads, billTos, markLoadInvoiced, markLoadPaid, revertInvoiceStage, askConfirm, companyInfo, setTab, editLoad, deleteLoad } = p;
  ...
  function handleEditLoad(l) { setOpenActionsFor(null); editLoad(l); setTab("loads"); }
  ...
  const filtered = useMemo(() => {
    ...
    return loads
      ...
      .filter((l) => !q || norm(String(l.loadNumber || "")).includes(q) || norm(l.workOrder).includes(q))
      .sort((a, b) => (b.deliveryDate || b.pickupDate || "").localeCompare(a.deliveryDate || a.pickupDate || ""));
  }, [loads, category, search]);
  ...
  return (
    <div>
      ...
      <div className="search-box">
        <Search size={14} color="#8A93A3" />
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by load #, invoice #, or work order…" />
      </div>
      ...
      <div className="trip-card-list">
        {filtered.map((l) => (
          <div className="trip-card" key={l.id} style={{ cursor: "default" }}>
            <div className="trip-card-top">
              <span className="trip-card-id">
                <span className="trip-number-pill">#{l.loadNumber}</span> <span className="trip-card-truck">· {l.billTo || "—"}</span>
                <span style={{ position: "relative", display: "inline-block", marginLeft: 6 }}>
                  <button type="button" className="mini-icon-btn" onClick={() => setOpenActionsFor(openActionsFor === l.id ? null : l.id)}><Pencil size={12} /></button>
                  {openActionsFor === l.id && (
                    <>
                      <div className="color-picker-backdrop" onClick={() => setOpenActionsFor(null)} />
                      <div className="account-menu-popover" style={{ minWidth: 150 }}>
                        <button type="button" className="btn secondary" style={{ marginTop: 0, width: "100%", marginBottom: 8 }} onClick={() => handleEditLoad(l)}>Edit Load</button>
                        <button type="button" className="btn danger" style={{ marginTop: 0, width: "100%" }} onClick={() => handleDeleteLoad(l)}>Delete Load</button>
                      </div>
                    </>
                  )}
                </span>
              </span>
              <span className="trip-card-meta trip-card-meta-bold">{fmtDate(l.deliveryDate || l.pickupDate)}</span>
            </div>
            <div style={{ fontSize: 11.5, color: "var(--text-dim)", marginBottom: 10 }}>{routeSummary(l)}{l.workOrder ? ` · WO ${l.workOrder}` : ""}</div>
            <div className="dash-card-body trip-stats-row">
              <div className="dash-card-hero"><span className="lbl">Rate</span><span className="val" style={{ color: "var(--accent)" }}>{compactMoney(l.rate)}</span></div>
              {category === "completed" && (
                <button className="btn" style={{ marginTop: 0, width: "auto", padding: "8px 16px" }} onClick={() => handleInvoice(l)}>Invoice</button>
              )}
              {category === "invoiced" && (
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <button className="btn secondary" style={{ marginTop: 0, width: "auto", padding: "8px 12px", fontSize: 11 }} onClick={() => setPrintingInvoice(l)}>View</button>
                  <button className="btn" style={{ marginTop: 0, width: "auto", padding: "8px 16px" }} onClick={() => markLoadPaid(l.id)}>Mark as Paid</button>
                </div>
              )}
              {category === "paid" && (
                <span style={{ fontSize: 11, color: "var(--green)", fontWeight: 700 }}>
                  Paid {l.paidAt ? fmtDate(l.paidAt.slice(0, 10)) : ""}
                </span>
              )}
            </div>
            {category === "invoiced" && (
              <div style={{ marginTop: 8 }}>
                <span className="mini-link" onClick={() => askConfirm("Move this load back to Completed (not invoiced)?", () => revertInvoiceStage(l.id, "none"))}>Revert to Completed</span>
              </div>
            )}
            {category === "paid" && (
              <div style={{ marginTop: 8 }}>
                <span className="mini-link" onClick={() => askConfirm("Move this load back to Invoiced (not yet paid)?", () => revertInvoiceStage(l.id, "invoiced"))}>Revert to Invoiced</span>
              </div>
            )}
          </div>
        ))}
...
function IftaCalculatorPage(p) {
  ...
  const driverHistorySorted = [...iftaReports].sort((a, b) => (b.year - a.year) || (b.quarter - a.quarter) || (b.savedAt || "").localeCompare(a.savedAt || ""));
  ...
  return (
    <div>
      ...
      {mode === "history" && (
        <div>
          {driverHistorySorted.length === 0 && <div className="empty-state">No saved IFTA reports yet.</div>}
          {driverHistorySorted.map((r) => {
            ...
            return (
              <div className="statement-card" key={r.id} onClick={() => setViewingReport(r)}>
                <div className="statement-card-date">Q{r.quarter} {r.year} — {r.truck === "ALL" ? "All Trucks" : `Truck ${r.truck}`}</div>
                <div className="statement-card-row">
