...
function StatsTab({ loads, trips, computeTripProfit, settings }) {
  ...
  function statsForRange(s, e) {
    ...
    const expenseFieldsNoRefunds = EXPENSE_REPORT_FIELDS.filter((f) => f.key !== "refunds");
    const totalExpenses = rangeTrips.reduce((sum, t) => sum + expenseFieldsNoRefunds.reduce((s2, f) => s2 + num(t[f.key]), 0), 0);
    const totalRefunds = rangeTrips.reduce((sum, t) => sum + num(t.refunds), 0);
    const companyProfit = grossRevenue - totalExpenses - totalRefunds;
    ...
    return { grossRevenue, totalMiles, totalExpenses, totalRefunds, companyProfit, totalLoadsCount, avgRPM, profitMargin, rows };
  }
  ...
  const dailySeries = useMemo(() => {
    ...
    return days.map((day) => {
      ...
      const dayExpenses = dayTrips.reduce((s, t) => s + expenseFieldsNoRefunds.reduce((s2, f) => s2 + num(t[f.key]), 0), 0);
      const dayRefunds = dayTrips.reduce((s, t) => s + num(t.refunds), 0);
      const dayLoadsCount = loads.filter((l) => l.status === "completed" && (l.deliveryDate || l.pickupDate) === day).length;
      return { day, gross, miles, loads: dayLoadsCount, profit: gross - dayExpenses - dayRefunds };
    });
