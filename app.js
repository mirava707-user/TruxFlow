...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .loads-new-load-btn { display: flex; align-items: center; gap: 8px; background: var(--surface); border: 1px solid var(--accent); border-radius: 10px; padding: 0 13px; height: 38px; color: var(--accent); font-family: 'Oswald', sans-serif; font-size: 12.5px; text-transform: uppercase; letter-spacing: 1px; cursor: pointer; white-space: nowrap; }
...
function LoadsTab(p) {
  ...
  return (
    <div>
      ...
      <div className="section-label no-line" style={{ marginTop: 8, gap: 10, width: "100%" }}>
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: "var(--surface)", border: "1px solid var(--accent)", borderRadius: 10, padding: "0 13px", height: 38 }}>
          <span>Loads</span>
          <button
            type="button"
            onClick={() => setSearchOpen((v) => !v)}
            style={{ background: searchOpen ? "var(--accent)" : "var(--surface-2)", border: "none", borderRadius: 8, width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}
          >
            <Search size={13} color={searchOpen ? "#1A1300" : "var(--text-dim)"} />
          </button>
        </div>
        <div style={{ flex: 1 }} />
        {!showLoadForm && (
          <button type="button" className="loads-new-load-btn" style={{ flex: 1, justifyContent: "center" }} onClick={openNewLoad}>
