...
const emptyDriver = () => ({ id: null, name: "", companyName: "", taxId: "", payType: "percent", rate: "", dispatchFeePercent: "", notes: "", active: true });
...
function normalizeDrivers(raw) {
  return raw.map((d) => (typeof d === "string"
    ? { id: uid(), name: d, companyName: "", taxId: "", payType: "percent", rate: "", dispatchFeePercent: "", notes: "", active: true }
    : { dispatchFeePercent: "", notes: "", active: true, taxId: "", ...d }));
}
          ...
          <div className="field-row">
            <div className="field">
              ...
              <div className="filter-row" style={{ flexWrap: "wrap", overflow: "visible" }}>
                <div className={`chip ${dashDriverFilter.length === 0 ? "active" : ""}`} onClick={() => setDashDriverFilter([])}>All Drivers</div>
                {driverNames.map((d) => (
                  <div key={d} className={`chip ${dashDriverFilter.includes(d) ? "active" : ""}`} onClick={() => toggleDriver(d)}>{d}</div>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="modal-overlay" onClick={() => setPrintingTaxDriver(null)}>
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
            <div className="print-area stub-sheet">
              ...
              <table className="stub-table">
                <tbody>
                  {printingTaxDriver.driver.taxId && <tr><td style={{ color: "#666" }}>EIN / SSN</td><td style={{ textAlign: "right" }}>{printingTaxDriver.driver.taxId}</td></tr>}
