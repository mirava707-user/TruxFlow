...
function FilterCard({ icon: Icon, label, value, options, onChange, fullWidth, compact, slim, hideLabel, accentBorder }) {
  const [open, setOpen] = useState(false);
  const current = options.find((o) => String(o.value) === String(value));
  const displayText = current ? (current.shortLabel || current.label) : "Select…";
  return (
    <div className={`dash-filter-card ${compact ? "dash-filter-card-compact" : ""} ${slim ? "dash-filter-card-slim" : ""}`} style={{ position: "relative", gridColumn: fullWidth ? "1 / -1" : undefined, border: accentBorder ? "1.5px solid var(--accent)" : undefined }}>
    ...
    <div>
      ...
      <div className="trips-month-card-v2">
        <div className="dash-filter-grid" style={{ marginBottom: 0 }}>
          <FilterCard
            icon={Calendar} label="Month" value={tripsMonth} onChange={(v) => setTripsMonth(parseInt(v, 10))} slim hideLabel accentBorder
            options={MONTH_NAMES.map((m, i) => {
              const g = grossByMonth[i + 1];
              return { value: i + 1, label: m, amountText: g > 0 ? money(g) : undefined };
            })}
          />
          <FilterCard
            icon={Calendar} label="Year" value={tripsYear} onChange={(v) => setTripsYear(parseInt(v, 10))} slim hideLabel accentBorder
            options={years.map((y) => {
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        {tripForm.truck && tripForm.startDate && tripForm.endDate && (() => {
          ...
          return (
            <span style={{ fontFamily: "-apple-system, BlinkMacSystemFont, sans-serif", fontSize: 17, fontWeight: 600, color: "var(--text)" }}>
