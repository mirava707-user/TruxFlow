...
function NotificationsSection() {
  ...
  const [status, setStatus] = useState("checking"); // checking | on | off | blocked | unsupported | needs-install
  ...
  const statusText = { checking: "Checking…", on: "On", off: "Off", blocked: "Blocked", unsupported: "Not available", "needs-install": "Needs Home Screen" }[status];
  return (
    <div>
      <div className="section-view-row" style={{ borderBottom: "none" }}>
        <div className="section-view-label">Filing Reminders</div>
        <div className="section-view-value" style={{ color: status === "on" ? "var(--green)" : undefined }}>{statusText}</div>
      </div>
      <div style={{ fontSize: 12, color: "var(--text-dim)", lineHeight: 1.5, margin: "2px 0 10px" }}>
        Oregon permit on the 15th, 25th and last day of the month (stops once marked filed), and IFTA 2 weeks, 3 days and on the due date. Sent at 9 AM.
      </div>
      {status === "needs-install" && (
        <div style={{ fontSize: 12.5, color: "var(--text)", lineHeight: 1.5 }}>
          On iPhone, reminders only work from the Home Screen app. In Safari tap Share → Add to Home Screen, open TruxFlow from there, then come back to this screen.
        </div>
      )}
      {status === "blocked" && (
        <div style={{ fontSize: 12.5, color: "var(--text)", lineHeight: 1.5 }}>
      ...
      {status === "off" && (
        <button type="button" className="btn" style={{ marginTop: 0 }} disabled={busy} onClick={turnOn}>{busy ? "Turning on…" : "Turn On Reminders"}</button>
      )}
      {status === "on" && (
        <div style={{ display: "flex", gap: 8 }}>
          <button type="button" className="btn" style={{ marginTop: 0, flex: 1 }} disabled={busy} onClick={sendTest}>Send Test</button>
          <button type="button" className="btn secondary" style={{ marginTop: 0, flex: 1 }} disabled={busy} onClick={turnOff}>Turn Off</button>
        </div>
      )}
