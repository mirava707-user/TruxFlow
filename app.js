      ...
      return (
        <div>
          ...
          {(r.loadSnapshots || []).map((l) => (
            <div className="history-item" key={l.loadId} style={{ cursor: "default" }}>
              <span>#{l.loadNumber} · {l.route}</span>
              <span style={{ fontFamily: "IBM Plex Mono, monospace", color: "var(--text)" }}>{money(l.driverPay)}</span>
            </div>
          ))}
