...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .field input[type="date"] {
    min-width: 0; max-width: 100%; box-sizing: border-box; font-size: 12px; padding: 10px 6px; overflow: hidden;
  }
  ...
  .date-input-clip input[type="date"] { border: none; background: transparent; }
...
function ExpenseDetail(p) {
  const { tripForm, setTripForm, activeTripId, truckNumbers, driverNames, computeTripMilesGross, computeTripProfit, saveTrip, deleteTrip, closeTripDetail, driverByName, askConfirm, nextTripNumber, settings, setTab, setTripLoadsFilter, trips, trucks, setDashSubTab, setStubTypeTab, setStubDriver, setViewingStubRecord, history } = p;
  ...
  if (mode === "generate") {
    ...
    return (
      <div>
        ...
        <div className="field-row">
          <div className="field"><label>Start</label><div className="date-input-clip"><input type="date" onClick={(e) => { try { e.target.showPicker(); } catch (_) {} }} value={genStart} onChange={(e) => setGenStart(e.target.value)} /></div></div>
          <div className="field"><label>End</label><div className="date-input-clip"><input type="date" onClick={(e) => { try { e.target.showPicker(); } catch (_) {} }} value={genEnd} onChange={(e) => setGenEnd(e.target.value)} /></div></div>
        </div>
      ...
      {stubDriver && stubMode === "generate" && (
        <>
          <div className="no-print">
            ...
            <div className="field-row">
              <div className="field"><label>Period Start</label><div className="date-input-clip"><input type="date" onClick={(e) => { try { e.target.showPicker(); } catch (_) {} }} value={stubStart} onChange={(e) => setStubStart(e.target.value)} /></div></div>
              <div className="field"><label>Period End</label><div className="date-input-clip"><input type="date" onClick={(e) => { try { e.target.showPicker(); } catch (_) {} }} value={stubEnd} onChange={(e) => setStubEnd(e.target.value)} /></div></div>
            </div>
