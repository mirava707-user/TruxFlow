...
function computeLoadPay(driver, load) {
  if (!driver) return 0;
  if (driver.payType === "cpm") return num(driver.rate) * num(load.loadedMiles);
...
function DispatchApp() {
  ...
  function openNewTrip() { setTripForm(emptyTrip()); setActiveTripId(null); setExpensesView("detail"); }
  ...
  function computeDriverPay(driverName, start, end) {
    ...
    DRIVER_DEDUCTION_FIELDS.forEach((f) => { expenseBreakdown[f.key] = driverTrips.reduce((s, t) => s + num(t[f.key]), 0); });
    ...
    return { driver, loadPays, grossPay, dispatchFeePercent, dispatchFee, expenseBreakdown, expenseTotal, refunds, netPay, trucksUsed, tripIds };
  }
  ...
  const dashReport = useMemo(() => {
    ...
    if (f.viewBy === "profit") {
      // Company profit view: dispatch fee earned per completed load, grouped by driver.
      const groups = {};
      scoped.forEach((l) => {
        const key = l.driver || "—";
        const driver = driverByName[l.driver];
        const feePct = dispatchFeePercentFor(driver);
        const fee = num(l.rate) * (feePct / 100);
        groups[key] = groups[key] || { loads: 0, gross: 0, dispatchFee: 0 };
        groups[key].loads += 1;
        groups[key].gross += num(l.rate);
        groups[key].dispatchFee += fee;
      });
      const rows = Object.entries(groups).map(([key, g]) => ({ key, loads: g.loads, gross: g.gross, dispatchFee: g.dispatchFee })).sort((a, b) => b.dispatchFee - a.dispatchFee);
  ...
  async function saveAndPrintStub() {
    ...
    const record = {
      ...
      grossPay: stub.grossPay, dispatchFeePercent: stub.dispatchFeePercent, dispatchFee: stub.dispatchFee,
      ...
      <div className="app-shell">
        ...
        <div className="content">
          ...
          {tab === "expenses" && expensesView === "list" && (
            <ExpensesList {...{ trips, computeTripProfit, openTrip, openNewTrip, tripsMonth, setTripsMonth, tripsYear, setTripsYear, settings }} />
          )}
          ...
          <div className="modal-overlay" onClick={() => setConfirmModal(null)}>
            <div className="modal-sheet" style={{ maxHeight: "none" }} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <button className="btn danger" style={{ marginTop: 0, flex: 1 }} onClick={() => { confirmModal.onConfirm(); setConfirmModal(null); }}>Delete</button>
              </div>
            </div>
          </div>
        )}
...
const STYLES = `
  ...
  .theme-light .row-line, .theme-light .list-row, .theme-light .history-item, .theme-light .trip-compact-row { border-color: #E4E7EC; }
  ...
  * { box-sizing: border-box; }
  ...
  @media print {
    ...
    .print-area.stub-sheet { border-radius: 0; padding: 0; }
...
function ExpensesList({ trips, computeTripProfit, openTrip, openNewTrip, tripsMonth, setTripsMonth, tripsYear, setTripsYear, settings }) {
  ...
  const rows = monthTrips.map((t) => ({ trip: t, ...computeTripProfit(t) }));
  const totalGross = rows.reduce((s, r) => s + r.gross, 0);
  const dispatchFeePct = resolveScheduledPercent(settings.dispatchFeeSchedule, tripsYear, tripsMonth, DEFAULT_DISPATCH_FEE);
  ...
  const dispatchFee = totalGross * (dispatchFeePct / 100);
    ...
    <div>
      ...
      <div className="trip-compact-list">
        <div className="trip-compact-header">
          <span className="col-trip">Trip</span>
          <span className="col-truck">Truck</span>
          <span className="col-date">Start</span>
          <span className="col-date">End</span>
          <span className="col-driver">D1</span>
          <span className="col-driver">D2</span>
          <span className="col-miles">Miles</span>
          <span className="col-gross">Gross</span>
        </div>
        {rows.length === 0 && <div className="empty-state">No trips recorded for {MONTH_NAMES[tripsMonth - 1]} {tripsYear}.</div>}
        {rows.map(({ trip: t, miles, gross }) => (
          <div className="trip-compact-row" key={t.id} onClick={() => openTrip(t)}>
            <span className="col-trip">{t.tripNumber}</span>
            <span className="col-truck">{t.truck}</span>
            <span className="col-date">{shortDate(t.startDate)}</span>
            <span className="col-date">{shortDate(t.endDate)}</span>
            <span className="col-driver" title={t.driver1}>{abbrevName(t.driver1)}</span>
            <span className="col-driver" title={t.driver2}>{abbrevName(t.driver2)}</span>
            <span className="col-miles">{miles.toLocaleString()}</span>
            <span className="col-gross">{compactMoney(gross)}</span>
          </div>
        ))}
      </div>
      ...
      <div className="monthly-summary-box">
        ...
        <div className="pay-summary-row"><span>Dispatch Fee ({dispatchFeePct}%)</span><span style={{ color: "var(--green)" }}>{money(dispatchFee)}</span></div>
      ...
      <div style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 12, lineHeight: 1.5 }}>
        Tap a row for full trip details. Miles and Gross update automatically from completed loads. Totals above are for reporting only.
      </div>
    </div>
  );
}
      ...
      {isExpenses ? (
        ...
        <>
          ...
          <div style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 10, lineHeight: 1.5 }}>
            Company profit shown here is the dispatch fee earned on each driver's completed loads (each driver's own %, defaulting to {DEFAULT_DISPATCH_FEE}%).
          </div>
        </>
      ) : (
      <div className="trip-compact-list">
...
function SettingsTab({ setFleetView, settings, saveStartingNumber, nextLoadNumber, saveSettings }) {
  ...
  return (
    <div>
      ...
      <div className="settings-card">
        <label style={{ fontSize: 10.5, textTransform: "uppercase", letterSpacing: 0.5, color: "var(--text-dim)", fontWeight: 600, display: "block", marginBottom: 4 }}>Business Percentages — Reporting Only</label>
        <div style={{ fontSize: 11, color: "var(--text-dim)", marginBottom: 14, lineHeight: 1.5 }}>
          Used by the Trips monthly summary and Dashboard reports only — this does not change individual driver pay rates or Pay Stub calculations.
        </div>
        <div style={{ fontSize: 11.5, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.4, color: "var(--accent)", marginBottom: 8 }}>Dispatch Fee</div>
        <PercentScheduleEditor label="Dispatch Fee" schedule={settings.dispatchFeeSchedule} defaultValue={DEFAULT_DISPATCH_FEE} onSave={(sched) => saveSettings({ dispatchFeeSchedule: sched })} />
        <div style={{ height: 1, background: "var(--border)", margin: "20px 0" }} />
        <div style={{ fontSize: 11.5, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.4, color: "var(--accent)", marginBottom: 8 }}>Dispatcher Pay</div>
...
// =====================================================================
// Simple local storage shim — no account, no backend, no setup.
// Drop-in replacement for window.storage using the browser's own
// localStorage, so every existing window.storage.get/.set call above
// works unchanged. Data lives on THIS device/browser only.
// =====================================================================
const STORAGE_PREFIX = "truxflow:";
window.storage = {
  async get(key) {
...
ReactDOM.createRoot(document.getElementById("root")).render(<DispatchApp />);
