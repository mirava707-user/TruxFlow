...
function StatsTab({ loads, trips, computeTripProfit, settings }) {
  ...
  function statsForRange(s, e) {
    ...
    // TRIP_EXPENSE_FIELDS already includes driverPay alongside fuel, permits, logbook,
    // insurance, other charges, advances, and truck pay — paying a driver is a real
    // company cost, so it belongs in the expense total same as everything else here.
    const totalExpenses = rangeTrips.reduce((sum, t) => sum + TRIP_EXPENSE_FIELDS.reduce((s2, f) => s2 + num(t[f.key]), 0), 0);
  ...
  const dailySeries = useMemo(() => {
    ...
    const dayMap = {};
    days.forEach((day) => { dayMap[day] = { day, gross: 0, miles: 0, loads: 0, profit: 0 }; });

    // Attribute each trip's full contribution to a single day within the visible
    // range — its end date if that's visible, otherwise its start date — so a
    // multi-day trip is counted exactly once rather than duplicated across every
    // day it touches. Trips are included by date-range overlap (not just an exact
    // startDate match) so a trip that began before this window but whose work
    // falls inside it isn't silently dropped from the totals.
    const rangeTrips = trips.filter((t) => t.startDate && overlaps(t.startDate, t.endDate || t.startDate, start, end));
    rangeTrips.forEach((t) => {
      let attribDay = t.endDate && dayMap[t.endDate] ? t.endDate : (dayMap[t.startDate] ? t.startDate : null);
      if (!attribDay) attribDay = start;
      const { gross, miles } = computeTripProfit(t);
      const expenses = TRIP_EXPENSE_FIELDS.reduce((s2, f) => s2 + num(t[f.key]), 0);
      dayMap[attribDay].gross += gross;
      dayMap[attribDay].miles += miles;
      dayMap[attribDay].profit += gross - expenses - num(t.refunds);
    });

    loads.forEach((l) => {
      if (l.status !== "completed") return;
      const ld = l.deliveryDate || l.pickupDate;
      if (dayMap[ld]) dayMap[ld].loads += 1;
    });
    ...
    <div>
      ...
      {!isAnnualTax && (dashViewBy === "dispatcher" ? (
        <div className="field-row">
          <div className="field">
            <label>Dispatcher</label>
            <select value={dashDispatcherFilter} onChange={(e) => setDashDispatcherFilter(e.target.value)}>
        ...
        <>
          ...
          <div className="field-row">
            <div className="field">
              ...
              <select value={dashBillToFilter} onChange={(e) => setDashBillToFilter(e.target.value)}>
                <option value="ALL">All</option>
