...
function DashboardTab(p) {
  ...
  function FilterCard({ icon: Icon, label, value, options, onChange, popoverKey, fullWidth }) {
    const current = options.find((o) => String(o.value) === String(value));
    return (
      <div className="dash-filter-card" style={{ position: "relative", gridColumn: fullWidth ? "1 / -1" : undefined }}>
        <button type="button" className="dash-filter-card-hit" onClick={() => setOpenFilterPopover(openFilterPopover === popoverKey ? null : popoverKey)}>
        ...
        {openFilterPopover === popoverKey && (
          <>
            <div className="color-picker-backdrop" onClick={() => setOpenFilterPopover(null)} />
            <div className="month-charge-popover dash-filter-popover">
              {options.map((opt) => (
                <div key={opt.value} className={`month-charge-item ${String(value) === String(opt.value) ? "selected" : ""}`} onClick={() => { onChange(opt.value); setOpenFilterPopover(null); }}>
                  <span>{opt.label}</span>
                </div>
              ))}
            </div>
