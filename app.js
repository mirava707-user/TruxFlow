...
function normalizeLoad(load) {
  if (load.stops && load.stops.length) {
    return { paidStatus: "unpaid", paidStubId: null, bolDataUri: "", bolFileName: "", bolType: "", invoiceStage: "none", invoicedAt: null, paidAt: null, dispatcher: "", shipperWarehouseCode: "", notes: "", dispatcherPaidStatus: "unpaid", dispatcherPaidStubId: null, ...load };
  }
...
const emptyDriver = () => ({ id: null, name: "", companyName: "", payType: "percent", rate: "", dispatchFeePercent: "", notes: "", active: true });
...
const emptyTrip = () => ({
  ...
  otherCharges: "", otherChargesList: [], refunds: "", truckPay: "",
  paidStatus: "unpaid", paidStubId: null,
});
function normalizeTrip(t) {
  const base = { paidStatus: "unpaid", paidStubId: null, driver2Color: "clear", otherChargesList: [], logbookMonth: "", insuranceMonth: "", orPermitNote: "", truckPayNote: "", ...t };
...
function normalizeDrivers(raw) {
  return raw.map((d) => (typeof d === "string"
    ? { id: uid(), name: d, companyName: "", payType: "percent", rate: "", dispatchFeePercent: "", notes: "", active: true }
    : { dispatchFeePercent: "", notes: "", active: true, ...d }));
}
...
function DispatchApp({ onSignOut, userEmail } = {}) {
  ...
  const [settings, setSettings] = useState({ startingLoadNumber: 1000, theme: "dark", companyName: "", companyAddress: "", dotNumber: "", companyEmail: "", dispatchFeeSchedule: [], dispatcherPaySchedule: [], companyLogoDataUri: "" });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const settingsDefaults = { startingLoadNumber: 1000, theme: "dark", companyName: "", companyAddress: "", dotNumber: "", companyEmail: "", dispatchFeeSchedule: [], dispatcherPaySchedule: [], companyLogoDataUri: "", pinCode: "" };
  ...
  function duplicateLoad(l) {
    setLoadForm({
      ...emptyLoad(),
      billTo: l.billTo, rate: l.rate, driver: l.driver, truck: l.truck,
  ...
  const filteredLoads = useMemo(() => {
    ...
    return list;
  }, [loads, filterTruck, filterStatus, loadSearch]);
  ...
  function computeDriverPay(driverName, start, end) {
    ...
    return { driver, loadPays, grossPay, isMileage, loadedMilesPay, emptyMilesPay, dispatchFeePercent, dispatchFee, expenseBreakdown, expenseTotal, refunds, netPay, trucksUsed, tripIds, otherChargeItems, insuranceMonths, logbookMonths, orPermitNotes, truckPayNotes };
  }
  ...
  const dashReport = useMemo(() => {
    ...
    if (f.viewBy === "dispatcher") {
      ...
      return {
        viewBy: f.viewBy, selectedName, dispatcher, grossTotal, loadCount, totalMiles, avgPerMile, pay,
        loads: matchingLoads.sort((a, b) => (b.deliveryDate || b.pickupDate || "").localeCompare(a.deliveryDate || a.pickupDate || "")),
  ...
  async function saveAndPrintStub() {
    ...
    const record = {
      ...
      orPermitNotes: stub.orPermitNotes || [], truckPayNotes: stub.truckPayNotes || [],
    ...
    <div className={`theme-${settings.theme || "dark"}`} style={{ background: "var(--bg)", minHeight: "100vh", fontFamily: "Inter, sans-serif" }}>
      ...
      <div className="app-shell">
          ...
          {tab === "dashboard" && (!settings.pinCode || dashUnlocked) && (
            <div>
              <div className="driver-subnav">
                <button className={`driver-subnav-btn ${dashSubTab === "reports" ? "active" : ""}`} onClick={() => setDashSubTab("reports")}>Reports</button>
                <button className={`driver-subnav-btn ${dashSubTab === "stub" ? "active" : ""}`} onClick={() => setDashSubTab("stub")}>Stub</button>
              </div>
              {dashSubTab === "reports" && (
                <DashboardTab {...{
              ...
              {dashSubTab === "stub" && (
                <div>
                  <div className="driver-subnav">
                    <button className={`driver-subnav-btn ${stubTypeTab === "driver" ? "active" : ""}`} onClick={() => setStubTypeTab("driver")}>Driver</button>
                    <button className={`driver-subnav-btn ${stubTypeTab === "dispatcher" ? "active" : ""}`} onClick={() => setStubTypeTab("dispatcher")}>Dispatcher</button>
                  </div>
                  {stubTypeTab === "driver" && (
                    <PayStubTab {...{
        ...
        <div className="tabbar no-print">
          <img src={WORDMARK_DATA_URI} alt="TruxFlow" className="sidebar-wordmark" />
...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .stub-loads-table { width: 100%; border-collapse: collapse; margin-bottom: 4px; }
  .stub-loads-table th, .stub-loads-table td { border: 1px solid var(--border); padding: 7px 8px; font-size: 11.5px; text-align: left; }
  ...
  .stub-total-gross-box span:last-child { font-family: 'IBM Plex Mono, monospace'; color: var(--accent); font-size: 16px; }
  ...
  .expense-report-row { display: flex; justify-content: space-between; align-items: center; padding: 11px 14px; border: 1px solid var(--border); border-radius: 8px; background: var(--surface); font-size: 13.5px; font-weight: 700; font-family: 'IBM Plex Mono', monospace; }
  ...
  .load-truck-plain { font-family: 'IBM Plex Mono', monospace; font-weight: 700; font-size: 13px; color: var(--text); flex-shrink: 0; }
  .load-dot-sep { color: var(--text-dim); flex-shrink: 0; }
  .load-dates-inline { font-size: 11.5px; color: var(--text-dim); font-family: 'IBM Plex Mono', monospace; font-weight: 600; flex-shrink: 0; }
  ...
  .manage-row { background: var(--surface); border: 1px solid var(--border); border-left: 4px solid #2DAA9E; border-radius: 8px; margin-bottom: 8px; overflow: hidden; box-shadow: 0 2px 6px rgba(0,0,0,0.12); }
  ...
  .acct-filter-row { display: flex; border: 1px solid var(--border); border-radius: 8px; overflow: hidden; margin-bottom: 18px; }
  .acct-filter-select { flex: 1; padding: 9px 6px; border: none; border-right: 1px solid var(--border); background: var(--surface); color: var(--text-dim); font-size: 12px; font-weight: 600; text-align: center; min-width: 0; }
  .acct-filter-select:last-child { border-right: none; }
  .ifta-table { border: 1px solid var(--border); border-left: 4px solid #E0793C; border-radius: 10px; overflow: hidden; margin-bottom: 16px; box-shadow: 0 2px 6px rgba(0,0,0,0.12); }
  ...
  .trip-table-header span { text-align: center; font-family: 'Oswald', sans-serif; font-size: 11.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.3px; color: #1A1300; }
  ...
  .tt-col-trip { width: 26px; flex-shrink: 0; }
  .tt-col-truck { width: 52px; flex-shrink: 0; }
  .tt-col-date { width: 54px; flex-shrink: 0; }
  ...
  .trip-table-gross { font-weight: 800; font-size: 16px; }
  ...
  .settings-card { background: var(--surface-2); border: 1px solid var(--border); border-left: 4px solid #8B6FD9; border-radius: 10px; padding: 14px; margin-bottom: 20px; box-shadow: 0 2px 6px rgba(0,0,0,0.12); }
  ...
  .dash-card { background: var(--surface); border: 1px solid var(--border); border-left: 4px solid #4A90D9; border-radius: 12px; padding: 12px 14px; box-shadow: 0 2px 6px rgba(0,0,0,0.12); }
  ...
  .fleet-menu-item { display: flex; align-items: center; gap: 12px; background: var(--surface); border: 1px solid var(--border); border-left: 4px solid #2DAA9E; border-radius: 10px; padding: 14px; margin-bottom: 10px; cursor: pointer; box-shadow: 0 2px 6px rgba(0,0,0,0.12); }
  ...
  .stub-header2 { border-bottom: 2px solid #1A1F27; padding-bottom: 14px; margin-bottom: 18px; }
  ...
  .stub-company2-line { font-size: 10px; color: #666; font-family: 'IBM Plex Mono', monospace; margin-top: 1px; }
  ...
  .stub-meta2-line { font-size: 10px; color: #666; font-family: 'IBM Plex Mono', monospace; margin-top: 2px; }
  .stub-title-row { display: flex; justify-content: space-between; align-items: baseline; padding-top: 12px; border-top: 1px solid #ddd; }
  ...
  return (
    <div>
      ...
      <div className="search-box">
        ...
        <input value={loadSearch} onChange={(e) => setLoadSearch(e.target.value)} placeholder="Search by load #, work order, bill to, or driver…" />
      </div>
        ...
        return (
          <div className="card" key={l.id} style={l.notes ? { borderRight: "3px solid #E15C4F" } : undefined}>
            <div className="card-head" onClick={() => setExpandedLoadId(open ? null : l.id)}>
              <div className="load-card-row1">
                <div className="load-card-row1-left">
                  <span style={{ fontFamily: "Oswald, sans-serif", fontWeight: 600, fontSize: 14, color: "var(--text)" }}>#{l.loadNumber}</span>
                  {l.workOrder && <span className="truck-standout">{l.workOrder}</span>}
                  <span className="load-truck-plain">{l.truck || "—"}</span>
                  <span className="load-dot-sep">•</span>
                  <span className="load-dates-inline">{mmdd(l.pickupDate)}-{mmdd(l.deliveryDate)}</span>
                </div>
                <span className={`status-pill ${l.status}`}>{l.status}</span>
              </div>
              <div className="load-card-row2">
                <span className="load-route-line">{routeSummary(l)}</span>
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                  <span style={{ fontFamily: "IBM Plex Mono, monospace", fontWeight: 700, fontSize: 15 }}>{money(l.rate)}</span>
...
function ExpensesList({ trips, computeTripProfit, computeTripFinancials, openTrip, openNewTrip, tripsMonth, setTripsMonth, tripsYear, setTripsYear, settings }) {
  ...
  const years = (() => { const y = new Date().getFullYear(); const arr = []; for (let i = y - 3; i <= Math.max(y + 1, 2030); i++) arr.push(i); return arr; })();
  ...
  return (
    <div>
      ...
      <div className="stop-card trips-month-card">
        <div className="field-row" style={{ marginBottom: 0 }}>
          <div className="field">
            <label>Month</label>
      ...
      <div className="trip-table">
        <div className="trip-table-header">
          <span className="tt-col-trip">Trip</span>
          <span className="trip-table-dot" style={{ opacity: 0 }}>·</span>
          <span className="tt-col-truck">Truck</span>
          <span className="trip-table-dot" style={{ opacity: 0 }}>·</span>
          <span className="tt-col-date">Date</span>
          <span className="trip-table-dot" style={{ opacity: 0 }}>·</span>
          <span className="tt-col-fee">$</span>
          <span className="trip-table-dot" style={{ opacity: 0 }}>·</span>
          <span className="tt-col-gross">Gross</span>
        </div>
        ...
        {rows.map(({ trip: t, miles, gross, fin }) => (
          <div className="trip-table-row" key={t.id} onClick={() => openTrip(t)}>
            <span className="tt-col-trip trip-table-num">{t.tripNumber}</span>
            <span className="trip-table-dot">·</span>
            <span className="tt-col-truck trip-table-truck">{t.truck || "—"}</span>
            <span className="trip-table-dot">·</span>
            <span className="tt-col-date trip-table-date">{shortDate(t.startDate)}</span>
            <span className="trip-table-dot">·</span>
            <span className="tt-col-fee" style={{ color: fin.mode === "profit" ? "var(--accent-2)" : "var(--green)" }}>
              {fin.mode === "profit" ? money(fin.profit) : money(fin.dispatchFee)}
            </span>
            <span className="trip-table-dot">·</span>
            <span className="tt-col-gross trip-table-gross" style={{ color: t.paidStatus === "paid" ? "var(--green)" : "var(--text)" }}>{money(gross)}</span>
          </div>
        ))}
      </div>
...
function ExpenseDetail(p) {
  const { tripForm, setTripForm, activeTripId, truckNumbers, driverNames, computeTripMilesGross, computeTripProfit, saveTrip, deleteTrip, closeTripDetail, driverByName, askConfirm } = p;
      ...
      <form onSubmit={saveTrip}>
          ...
          return (
            <>
              ...
              <div className="field-row">
                ...
                <div className="field">
                  <label>Logbook Month</label>
                  <select value={tripForm.logbookMonth || ""} onChange={(e) => setTripForm({ ...tripForm, logbookMonth: e.target.value })}>
                    <option value="">—</option>
              ...
              <div className="field-row">
                ...
                <div className="field">
                  ...
                  <select value={tripForm.insuranceMonth || ""} onChange={(e) => setTripForm({ ...tripForm, insuranceMonth: e.target.value })}>
                    <option value="">—</option>
                    {MONTH_ABBR.map((m) => <option key={m} value={m}>{m}</option>)}
                  </select>
                </div>
        ...
        {tripForm.truck && tripForm.startDate && tripForm.endDate && (() => {
          ...
          if (isProfitCase) {
            ...
            const profit = gross - driverEarnings - expensesForPay + refunds;
            return (
...
function DashboardTab(p) {
  ...
  const [expandedDispatchLoads, setExpandedDispatchLoads] = useState({});

  function toggleDriver(name) {
    setDashDriverFilter((prev) => prev.includes(name) ? prev.filter((n) => n !== name) : [...prev, name]);
  }
  function toggleDispatchLoad(id) {
    setExpandedDispatchLoads((prev) => ({ ...prev, [id]: !prev[id] }));
  }
  function expandAllDispatchLoads() {
    const all = {};
    (dashReport.loads || []).forEach((l) => { all[l.id] = true; });
    setExpandedDispatchLoads(all);
  }
  function collapseAllDispatchLoads() { setExpandedDispatchLoads({}); }
    ...
    <div>
      ...
      {dashViewBy === "dispatcher" ? (
        <div className="field-row">
          <div className="field">
            <label>Dispatcher</label>
            <select value={dashDispatcherFilter} onChange={(e) => setDashDispatcherFilter(e.target.value)} style={{ fontFamily: "Inter" }}>
              <option value="">Select a dispatcher…</option>
              {dispatchers.map((d) => <option key={d.id} value={d.name}>{d.name}{d.position === "main" ? " (Main)" : ""}</option>)}
            </select>
          </div>
          <div className="field">
            <label>Truck</label>
          ...
          <>
            ...
            <div className="section-label-row">
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <span className="mini-link" onClick={expandAllDispatchLoads}>Expand All</span>
                <span className="mini-link" onClick={collapseAllDispatchLoads}>Collapse All</span>
              </div>
            </div>
            ...
            <div className="trip-card-list">
              {dashReport.loads.map((l) => {
                ...
                const open = !!expandedDispatchLoads[l.id];
                return (
                  <div className="trip-card" key={l.id} onClick={() => toggleDispatchLoad(l.id)}>
                    <div className="trip-card-top">
                      <span className="trip-card-id"><span className="trip-number-pill">#{l.loadNumber}</span> <span className="trip-card-truck truck-standout">· {l.billTo || "—"}</span></span>
                      <span className="trip-card-meta trip-card-meta-bold trip-date-lg">{fmtDate(l.deliveryDate || l.pickupDate)}</span>
                    </div>
                    <div className="dash-card-body trip-stats-row">
                      <div className="dash-card-hero"><span className="lbl">Line Haul</span><span className="val" style={{ color: "var(--accent)" }}>{compactMoney(l.rate)}</span></div>
                      <span className={`status-pill ${l.status}`}>{l.status}</span>
                    </div>
                    {open && (
                      <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px dashed var(--border)" }} onClick={(e) => e.stopPropagation()}>
                        <div className="row-line"><span>Work Order</span><span>{l.workOrder || "—"}</span></div>
                        <div className="row-line"><span>Shipper (Pickup)</span><span>{cityState(l.shipperCity, l.shipperState) || l.shipperName || "—"}</span></div>
                        <div className="row-line"><span>Receiver (Delivery)</span><span>{finalStop ? (cityState(finalStop.city, finalStop.state) || finalStop.receiverName || "—") : "—"}</span></div>
                        <div className="row-line"><span>Load Date</span><span>{fmtDate(l.deliveryDate || l.pickupDate)}</span></div>
                      </div>
                    )}
                  </div>
                );
          ...
          {dashReport.rows.map((r) => (
            <div className="dash-card" key={r.key}>
              <div className="dash-card-top">
                ...
                <span className="dash-card-badge">{r.loads} load{r.loads === 1 ? "" : "s"}</span>
              </div>
              <div className="dash-card-body">
                <div className="dash-card-hero"><span className="lbl">Miles</span><span className="val" style={{ color: "var(--accent)" }}>{r.miles.toLocaleString()}</span></div>
                <div className="dash-card-hero"><span className="lbl">Empty</span><span className="val">{r.emptyMiles.toLocaleString()}</span></div>
              </div>
              <div className="dash-card-stats">
                <span>Gross <strong>{compactMoney(r.gross)}</strong></span>
                <span>$/mi <strong>${r.revPerMile.toFixed(2)}</strong></span>
              </div>
            </div>
          ))}
          {dashReport.rows.length > 0 && (
            <div className="dash-card dash-summary-card">
              <div className="dash-card-top">
                ...
                <span className="dash-card-badge">{dashReport.totals.loads} loads</span>
              </div>
              <div className="dash-card-body">
                <div className="dash-card-hero"><span className="lbl">Total Miles</span><span className="val" style={{ color: "var(--accent)" }}>{dashReport.totals.miles.toLocaleString()}</span></div>
                <div className="dash-card-hero"><span className="lbl">Total Empty</span><span className="val">{dashReport.totals.emptyMiles.toLocaleString()}</span></div>
              </div>
              <div className="dash-card-stats">
                <span>Gross <strong>{compactMoney(dashReport.totals.gross)}</strong></span>
                <span>$/mi <strong>${dashReport.totals.revPerMile.toFixed(2)}</strong></span>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="dash-card-list">
            ...
            {dashReport.rows.map((r) => (
              <div className="dash-card" key={r.key}>
                ...
                <div className="dash-card-body">
                  <div className="dash-card-hero"><span className="lbl">Gross</span><span className="val">{compactMoney(r.gross)}</span></div>
                  <div className="dash-card-hero"><span className="lbl">Profit</span><span className="val" style={{ color: "var(--green)" }}>{compactMoney(r.dispatchFee)}</span></div>
                </div>
              </div>
            ))}
            {dashReport.rows.length > 0 && (
              <div className="dash-card dash-summary-card">
                ...
                <div className="dash-card-body">
                  <div className="dash-card-hero"><span className="lbl">Total Gross</span><span className="val">{compactMoney(dashReport.totals.gross)}</span></div>
                  <div className="dash-card-hero"><span className="lbl">Total Profit</span><span className="val" style={{ color: "var(--green)" }}>{compactMoney(dashReport.totals.dispatchFee)}</span></div>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="dash-card-list">
        ...
        {dashReport.rows.map((r) => (
          <div className="dash-card" key={r.key}>
            ...
            {!isBillTo && (
              <div className="dash-card-stats">
                <span>Miles <strong>{r.miles.toLocaleString()}</strong></span>
                <span>Empty <strong>{r.emptyMiles ? r.emptyMiles.toLocaleString() : "-"}</strong></span>
                <span>$/mi <strong>${r.revPerMile.toFixed(2)}</strong></span>
              </div>
            )}
          </div>
        ...
        {dashReport.rows.length > 0 && (
          <div className="dash-card dash-summary-card">
            ...
            {!isBillTo && (
              <div className="dash-card-stats">
                <span>Miles <strong>{dashReport.totals.miles.toLocaleString()}</strong></span>
                <span>Empty <strong>{dashReport.totals.emptyMiles ? dashReport.totals.emptyMiles.toLocaleString() : "-"}</strong></span>
                <span>$/mi <strong>${dashReport.totals.revPerMile.toFixed(2)}</strong></span>
              </div>
            )}
          </div>
      ...
      {!isProfit && !isExpenses && !isMiles && !isDispatcher && (
      <div style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 10, lineHeight: 1.5 }}>
      ...
      <div>
        ...
        <div className="print-area stub-sheet">
          <div className="stub-header2">
            <div className="stub-header2-top">
              <div className="stub-brand">
                {companyInfo && companyInfo.companyLogoDataUri && <img src={companyInfo.companyLogoDataUri} alt="Company logo" className="stub-logo2" />}
              ...
              <div className="stub-meta2">
                <div className="stub-meta2-num">Statement #{String(r.stubNumber || 0).padStart(4, "0")}</div>
            ...
            <div className="stub-title-row">
              ...
              <div className="stub-driver2">{r.dispatcherName}</div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 16 }}>
            <table className="stub-loads-table" style={{ flex: 1 }}>
              <thead><tr><th>WO#</th><th>Date</th><th style={{ textAlign: "right" }}>Rate</th></tr></thead>
              <tbody>
                {col1.map((l) => (
                  <tr key={l.loadId}><td>{l.workOrder || l.loadNumber}</td><td>{mmdd(l.date)}</td><td style={{ textAlign: "right" }}>{money(l.rate)}</td></tr>
                ))}
              </tbody>
            </table>
            <table className="stub-loads-table" style={{ flex: 1 }}>
          ...
          <div className="stub-summary">
            <table>
              <tbody>
                <tr><td>Total Gross</td><td style={{ textAlign: "right" }}>{money(r.grossTotal)}</td></tr>
                <tr><td>Pay Method</td><td style={{ textAlign: "right" }}>{r.payMethod === "flat" ? `$${num(r.payValue).toFixed(2)}/load` : `${r.payValue}%`}</td></tr>
                <tr className="net-row"><td>Total Earnings</td><td style={{ textAlign: "right" }}>{money(r.earnings)}</td></tr>
              </tbody>
            </table>
          </div>
        </div>
  ...
  if (mode === "generate") {
    ...
    return (
      <div>
        ...
        <div className="field-row">
          <div className="field"><label>Start</label><input type="date" value={genStart} onChange={(e) => setGenStart(e.target.value)} /></div>
          <div className="field"><label>End</label><input type="date" value={genEnd} onChange={(e) => setGenEnd(e.target.value)} /></div>
        </div>
            ...
            <>
              ...
              {unpaidLoads.map((l) => (
                <div className="history-item" key={l.id} style={{ cursor: "default" }}>
                  <span>#{l.loadNumber}{l.workOrder ? ` · WO ${l.workOrder}` : ""} · {fmtDate(l.deliveryDate || l.pickupDate)}</span>
        ...
        <div className="print-area stub-sheet">
          <div className="stub-header2">
            <div className="stub-header2-top">
              ...
              <div className="stub-meta2">
                <div className="stub-meta2-num">Pay #{String(r.stubNumber || 0).padStart(4, "0")}</div>
                <div className="stub-meta2-line">{r.generatedAt ? fmtDate(r.generatedAt.slice(0, 10)) : ""}</div>
                {r.trucksUsed && r.trucksUsed.length ? <div className="stub-meta2-line">Truck{r.trucksUsed.length > 1 ? "s" : ""} {r.trucksUsed.join(", ")}</div> : null}
              </div>
            </div>
            <div className="stub-title-row">
              <div className="stub-title2">Payment Statement</div>
              <div className="stub-driver2">{r.displayedAs || r.driverName}</div>
            </div>
          </div>
          <table className="stub-table">
          ...
          <div className="stub-summary">
            <table>
              <tbody>
                ...
                {r.dispatchFee > 0 && <tr className="fee-row"><td>Dispatch Fee ({r.dispatchFeePercent}%)</td><td style={{ textAlign: "right" }}>-{money(r.dispatchFee)}</td></tr>}
                {DRIVER_DEDUCTION_FIELDS.filter((f) => f.key !== "otherCharges").map((f) => (r.expenseBreakdown && r.expenseBreakdown[f.key] > 0) ? (
                  <tr className="fee-row" key={f.key}>
                    ...
                    <td style={{ textAlign: "right" }}>-{money(r.expenseBreakdown[f.key])}</td>
                  </tr>
                ) : null)}
                {(r.otherChargeItems || []).map((item, i) => (
                  <tr className="fee-row" key={`oc${i}`}><td>{item.note}</td><td style={{ textAlign: "right" }}>-{money(item.amount)}</td></tr>
                ))}
        ...
        <div className="no-print">
          ...
          {historySubTab === "unpaid" && (
            <>
              ...
              {unpaidLoads.map((l) => (
                <div className="history-item" key={l.id} style={{ cursor: "default" }}>
                  <span>#{l.loadNumber} · {l.billTo} · {fmtDate(l.deliveryDate || l.pickupDate)}</span>
                  <span style={{ fontFamily: "IBM Plex Mono, monospace", color: "var(--text)" }}>{money(l.rate)}</span>
                </div>
              ))}
            </>
              ...
              <div className="print-area stub-sheet" style={{ marginTop: 16 }}>
                <div className="stub-header2">
                  <div className="stub-header2-top">
                    ...
                    <div className="stub-meta2">
                      <div className="stub-meta2-num">{fmtDate(stubStart)} – {fmtDate(stubEnd)}</div>
                      <div className="stub-meta2-line">{stub.trucksUsed.length ? `Truck${stub.trucksUsed.length > 1 ? "s" : ""} ${stub.trucksUsed.join(", ")}` : "No truck assigned"}</div>
                    </div>
                  </div>
                  <div className="stub-title-row">
                    <div className="stub-title2">Payment Statement</div>
                    <div className="stub-driver2">{resolveDisplayName(stub.driver, stubDisplayAs)}</div>
                  </div>
                </div>
                <table className="stub-table">
                ...
                <div className="stub-summary">
                  <table>
                    <tbody>
                      ...
                      {stub.dispatchFee > 0 && <tr className="fee-row"><td>Dispatch Fee ({stub.dispatchFeePercent}%)</td><td style={{ textAlign: "right" }}>-{money(stub.dispatchFee)}</td></tr>}
                      {DRIVER_DEDUCTION_FIELDS.filter((f) => f.key !== "otherCharges").map((f) => stub.expenseBreakdown[f.key] > 0 ? (
                        <tr className="fee-row" key={f.key}>
                          ...
                          <td style={{ textAlign: "right" }}>-{money(stub.expenseBreakdown[f.key])}</td>
                        </tr>
                      ) : null)}
      ...
      {items.map((it) => (
        <div className="fleet-menu-item" key={it.key} onClick={() => setFleetView(it.key)}>
          ...
          <div className="fleet-menu-text">
            ...
            <div className="count">{typeof it.count === "number" ? `${it.count} record${it.count === 1 ? "" : "s"}` : it.description}</div>
          </div>
    ...
    <div>
      ...
      <form onSubmit={saveTruck}>
        ...
        <div className="field-row">
          ...
          <div className="field">
            <label>Status</label>
            <select value={truckForm.active === false ? "inactive" : "active"} onChange={(e) => setTruckForm({ ...truckForm, active: e.target.value === "active" })}>
...
function AutocompleteInput({ value, onChange, onSelect, options, placeholder, getLabel, getSub }) {
  ...
  const q = norm(value);
  const filtered = (q ? options.filter((o) => norm(getLabel(o)).includes(q)) : options).slice(0, 6);
  return (
    <div style={{ position: "relative" }}>
      <input
        type="text"
      ...
      {focused && filtered.length > 0 && (
        <div className="autocomplete-dropdown">
          {filtered.map((o, i) => (
            <div className="autocomplete-item" key={i} onMouseDown={() => onSelect(o)}>
    ...
    <div>
      ...
      <form onSubmit={saveDriver}>
        ...
        <div className="field-row">
          ...
          <div className="field">
            <label>Status</label>
            <select value={driverForm.active === false ? "inactive" : "active"} onChange={(e) => setDriverForm({ ...driverForm, active: e.target.value === "active" })}>
              <option value="active">Active</option>
              <option value="inactive">Inactive</option>
            </select>
          </div>
    ...
    <div>
      ...
      <form onSubmit={saveBillTo}>
        ...
        <div className="field-row">
          ...
          <div className="field">
            <label>Status</label>
            <select value={billToForm.active === false ? "inactive" : "active"} onChange={(e) => setBillToForm({ ...billToForm, active: e.target.value === "active" })}>
              <option value="active">Active</option>
              <option value="inactive">Inactive</option>
            </select>
          </div>
      ...
      <form onSubmit={saveDispatcher}>
        ...
        <div className="field-row">
          <div className="field">
            ...
            <select value={dispatcherForm.payMethod} onChange={(e) => setDispatcherForm({ ...dispatcherForm, payMethod: e.target.value })}>
              <option value="percent">Percentage</option>
              <option value="flat">Flat Pay Per Load</option>
            </select>
          </div>
          <div className="field">
        ...
        <div className="field-row">
          <div className="field">
            ...
            <select value={dispatcherForm.position} onChange={(e) => setDispatcherForm({ ...dispatcherForm, position: e.target.value })}>
              <option value="dispatcher">Dispatcher</option>
              <option value="main">Main Dispatcher</option>
            </select>
          </div>
          <div className="field">
            <label>Status</label>
            <select value={dispatcherForm.active ? "active" : "inactive"} onChange={(e) => setDispatcherForm({ ...dispatcherForm, active: e.target.value === "active" })}>
              <option value="active">Active</option>
              <option value="inactive">Inactive</option>
            </select>
          </div>
...
function AccountingPage(p) {
  ...
  const filtered = useMemo(() => {
    ...
    return loads
      ...
      .sort((a, b) => (b.deliveryDate || b.pickupDate || "").localeCompare(a.deliveryDate || a.pickupDate || ""));
  ...
  return (
    <div>
      ...
      <div className="acct-filter-row">
        <select className="acct-filter-select" value={truckFilter} onChange={(e) => setTruckFilter(e.target.value)}>
          <option value="ALL">All Trucks</option>
          {truckNumbers.map((t) => <option key={t} value={t}>{t}</option>)}
        </select>
        <select className="acct-filter-select" value={monthFilter} onChange={(e) => setMonthFilter(Number(e.target.value))}>
          <option value={0}>All Months</option>
          {MONTH_NAMES.map((m, i) => <option key={m} value={i + 1}>{m}</option>)}
        </select>
        <select className="acct-filter-select" value={yearFilter} onChange={(e) => setYearFilter(Number(e.target.value))}>
            ...
            <div className="card-head" onClick={() => setExpandedId(open ? null : l.id)}>
              <div className="load-card-row1">
                ...
                <div onClick={(e) => e.stopPropagation()}>
                  {category === "completed" && (
                    <button className="btn" style={{ marginTop: 0, width: "auto", padding: "6px 14px", fontSize: 12 }} onClick={() => handleInvoice(l)}>Invoice</button>
                  )}
                  {category === "invoiced" && (
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      <button className="btn secondary" style={{ marginTop: 0, width: "auto", padding: "6px 10px", fontSize: 11 }} onClick={() => setPrintingInvoice(l)}>View</button>
                      <button className="btn" style={{ marginTop: 0, width: "auto", padding: "6px 14px", fontSize: 12 }} onClick={() => markLoadPaid(l.id)}>Mark as Paid</button>
                    </div>
                  )}
                  {category === "paid" && (
                    <span className="status-pill completed">Paid {l.paidAt ? fmtDate(l.paidAt.slice(0, 10)) : ""}</span>
                  )}
                </div>
              </div>
              <div className="load-card-row2">
                <span className="load-route-line">{routeSummary(l)}</span>
                <span style={{ fontFamily: "IBM Plex Mono, monospace", fontWeight: 700, fontSize: 15, color: "var(--accent)", flexShrink: 0 }}>{money(l.rate)}</span>
              </div>
            </div>
            {open && (
              <div className="card-detail">
  ...
  return (
    <div>
      ...
      {mode === "history" && (
        <div>
          <div className="acct-filter-row">
            <select className="acct-filter-select" value={histTruckFilter} onChange={(e) => setHistTruckFilter(e.target.value)}>
              <option value="ALL">All Trucks</option>
              {truckNumbers.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
            <select className="acct-filter-select" value={histQuarterFilter} onChange={(e) => setHistQuarterFilter(Number(e.target.value))}>
            ...
            <select className="acct-filter-select" value={histYearFilter} onChange={(e) => setHistYearFilter(Number(e.target.value))}>
              <option value={0}>All Years</option>
      ...
      {mode === "build" && (
        <>
          <div className="field-row">
            <div className="field">
              <label>Truck</label>
              <select value={report.truck} onChange={(e) => setReport({ ...report, truck: e.target.value })}>
                <option value="ALL">All Trucks (Combined)</option>
                {truckNumbers.map((t) => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
          </div>
          <div className="field-row">
            <div className="field">
              <label>Quarter</label>
              <select value={report.quarter} onChange={(e) => setReport({ ...report, quarter: Number(e.target.value) })}>
