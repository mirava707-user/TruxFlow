        ...
        <form onSubmit={saveLoad} className="load-entry-form">
          <div className="field-row">
            <div className="field">
              ...
              >
                <ChevronLeft size={16} />
                #{editingLoadId ? loadForm.loadNumber : nextLoadNumber}
              </div>
            </div>
...
function DashboardTab(p) {
  ...
  const [orMonthPickerOpen, setOrMonthPickerOpen] = useState(false);
      ...
      {!isAnnualTax && (dashViewBy === "dispatcher" ? (
        ...
        <>
          ...
          <div className="dash-filter-grid">
            <div className="field" style={{ position: "relative" }}>
              <label>Month</label>
              <button type="button" className="month-select-btn trips-top-field" onClick={() => setOrMonthPickerOpen((v) => !v)}>
                <span>{MONTH_NAMES[orSelMonth - 1]}</span>
                <ChevronDown size={14} />
              </button>
              {orMonthPickerOpen && (
                <>
                  <div className="color-picker-backdrop" onClick={() => setOrMonthPickerOpen(false)} />
                  <div className="month-charge-popover">
                    {MONTH_NAMES.map((m, i) => {
                      ...
                      return (
                        <div key={m} className={`month-charge-item ${orSelMonth === i + 1 ? "selected" : ""}`} onClick={() => { setOrMonthYear(orSelYear, i + 1); setOrMonthPickerOpen(false); }}>
                          <span>{m}</span>
                          {!!paidAmt && (
                            <span className="month-charge-amt" style={filed ? { color: "var(--green)" } : undefined}>
                              {money(paidAmt)}{filed ? " ✓" : ""}
                            </span>
                          )}
                        </div>
