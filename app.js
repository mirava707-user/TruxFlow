...
async function generatePdf(filename) {
  ...
  try {
    ...
    // Measure, from the live clone, the header block (company/driver/dispatcher info at
    // the top) and every row-like element's bottom edge — these become the only places
    // a page break is allowed to land, so a page break can never cut through a row's
    // text the way a blind fixed-height slice could.
    const cloneTop = clone.getBoundingClientRect().top;
    const headerEl = clone.querySelector(".stub-header2");
    const headerHeightCss = headerEl ? headerEl.getBoundingClientRect().height : 0;
