            ...
            <div>
              ...
              {dashSubTab === "reports" && (
                <DashboardTab {...{
                  ...
                  setTab, setTripLoadsFilter, saveSettings, trips,
                }} />
              )}
              {dashSubTab === "stub" && (
                <div>
...
function ExpensesList({ trips, computeTripProfit, computeTripFinancials, openTrip, openNewTrip, openNewPendingTrip, tripsMonth, setTripsMonth, tripsYear, setTripsYear, settings, truckNumbers, loads }) {
  ...
  // Oregon Permit total for this month — mirrors the Dash → Reports → Oregon
  // Permit calculation exactly (completed loads' orMiles within the month,
  // times the per-mile rate), so the two pages always agree on the number.
  const orPermitAmount = (loads || [])
    ...
    <div>
      ...
      <form onSubmit={saveTrip}>
        ...
        <div className="field-row date-row">
          <div className="field"><label>Start Date</label><input type="date" min={minStartDate} onClick={(e) => { try { e.target.showPicker(); } catch (_) {} }} value={tripForm.startDate} onChange={(e) => handleStartDateChange(e.target.value)} /></div>
      ...
      {isAnnualTax ? (
        ...
        !dashReport.selectedName ? (
          ...
          <>
            <div className="stub-total-gross-box" style={{ marginTop: 0, marginBottom: 12 }}>
              <span>Total Gross ({fmtDate(dashReport.start)} – {fmtDate(dashReport.end)})</span>
        ...
        <>
          ...
          {dashReport.rows.length > 0 && (() => {
            ...
            const orFiled = !!(companyInfo.oregonPermitFiled && companyInfo.oregonPermitFiled[orMonthKey]);
            function toggleOrFiled() {
              const next = { ...(companyInfo.oregonPermitFiled || {}) };
              if (orFiled) delete next[orMonthKey];
              else next[orMonthKey] = true;
              ...
              <>
                ...
                {orFiled && (
                  <div style={{ fontSize: 11.5, color: "var(--green)", marginTop: 8, fontWeight: 600 }}>
                    ✓ Oregon Permit is filed for {orMonthLabel}.
                  </div>
                )}
                {(() => {
                  ...
                  const inputStyle = { width: 110, textAlign: "right", background: "#FFFFFF", color: "#14181F", border: "1px solid var(--border)", borderRadius: 8, padding: "7px 9px", fontSize: 13, fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600 };
                  return (
                      ...
                      <div style={rowStyle}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <button
                            ...
                            style={orFiled
                              ? { display: "flex", alignItems: "center", gap: 5, background: "var(--green)", border: "none", borderRadius: 8, padding: "7px 11px", color: "#0E2318", fontFamily: "'Oswald', sans-serif", fontWeight: 700, fontSize: 11.5, cursor: "pointer", flexShrink: 0 }
                              : { display: "flex", alignItems: "center", gap: 5, background: "transparent", border: "1px solid var(--border)", borderRadius: 8, padding: "7px 11px", color: "var(--text-dim)", fontFamily: "'Oswald', sans-serif", fontWeight: 700, fontSize: 11.5, cursor: "pointer", flexShrink: 0 }}
                          >
