        ...
        <div className="team-roster-list">
          {messages.map((m) => (
            <div className="team-roster-row" key={m.id} style={{ flexDirection: "column", alignItems: "stretch" }}>
              <div style={{ fontSize: 13, color: "var(--text)", lineHeight: 1.5, marginBottom: 6 }}>{m.message}</div>
              <div className="team-roster-role">{m.email}{m.is_me ? " (you)" : ""} · {new Date(m.created_at).toLocaleDateString(undefined, { month: "short", day: "numeric" })}</div>
            </div>
          ))}
        </div>
      )}
...
function AccountSidebar({ userEmail, onSignOut, onClose, settings, saveSettings, saveStartingNumber, nextLoadNumber, askConfirm }) {
  const hasBackend = typeof sb !== "undefined";
        ...
        {view === "menu" && (
          <div className="account-sidebar-menu">
            ...
            <button type="button" className="account-sidebar-menu-item" onClick={() => setView("help")}>
              <MessageCircle size={17} /> <span>Help & Support</span> <ChevronRight size={16} className="account-sidebar-menu-arrow" />
            </button>
            <div style={{ flex: 1 }} />
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div style={{ position: "relative" }}>
                ...
                {accountModalOpen && onSignOut && (
                  <AccountSidebar
                    userEmail={userEmail} onSignOut={onSignOut} onClose={() => setAccountModalOpen(false)}
