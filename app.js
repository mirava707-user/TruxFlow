...
function normalizeLoad(load) {
  if (load.stops && load.stops.length) {
    return { paidStatus: "unpaid", paidStubId: null, bolDataUri: "", bolFileName: "", bolType: "", invoiceStage: "none", invoicedAt: null, paidAt: null, dispatcher: "", ...load };
  }
  const stop = { id: uid(), receiverName: load.receiverName || "", city: load.receiverCity || "", state: load.receiverState || "", zip: load.receiverZip || "" };
  return { ...load, stops: [stop], paidStatus: load.paidStatus || "unpaid", paidStubId: load.paidStubId || null, bolDataUri: load.bolDataUri || "", bolFileName: load.bolFileName || "", bolType: load.bolType || "", invoiceStage: load.invoiceStage || "none", invoicedAt: load.invoicedAt || null, paidAt: load.paidAt || null, dispatcher: load.dispatcher || "" };
}
...
function DispatchApp({ onSignOut, userEmail } = {}) {
  ...
  function duplicateLoad(l) {
    setLoadForm({
      ...
      shipperName: l.shipperName, shipperCity: l.shipperCity, shipperState: l.shipperState, shipperZip: l.shipperZip,
        ...
        <div className={`content ${textScale === "large" ? "text-large" : ""}`}>
          ...
          {tab === "fleet" && fleetView === "accounting" && (
            <AccountingPage {...{
              ...
              askConfirm, companyInfo: settings,
            }} />
          )}
        </div>
            ...
            <div className="card-head" onClick={() => setExpandedLoadId(open ? null : l.id)}>
              <div className="load-card-row1">
                <div className="load-card-row1-left">
                  ...
                  <span className="load-dot-sep">•</span>
                  <span className="load-dates-inline">{shortDate(l.pickupDate)} - {shortDate(l.deliveryDate)}</span>
                </div>
...
function AccountingPage(p) {
  const { setFleetView, loads, billTos, markLoadInvoiced, markLoadPaid, revertInvoiceStage, askConfirm, companyInfo } = p;
      ...
      <div className="trip-card-list">
        {filtered.map((l) => (
          <div className="trip-card" key={l.id} style={{ cursor: "default" }}>
            <div className="trip-card-top">
              <span className="trip-card-id"><span className="trip-number-pill">#{l.loadNumber}</span> <span className="trip-card-truck">· {l.billTo || "—"}</span></span>
