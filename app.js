...
async function generatePdf(filename) {
  ...
  try {
    ...
    const usableHeightPt = pageHeight - MARGIN_Y * 2;
    // Slice the source canvas into page-sized chunks ourselves, rather than relying
    // on jsPDF to clip a single tall image at each page boundary — that clips at the
    // raw physical edge (ignoring our margin), so the app's own bookkeeping of "how
    // much was shown" drifted from reality and pages ended up overlapping content.
    const pxPerPt = canvas.width / imgWidthPt;
    const sliceHeightPx = Math.floor(usableHeightPt * pxPerPt);
    let renderedPx = 0;
    let firstPage = true;
    while (renderedPx < canvas.height) {
      ...
      sctx.drawImage(canvas, 0, renderedPx, canvas.width, thisSliceHeightPx, 0, 0, canvas.width, thisSliceHeightPx);
      const sliceData = sliceCanvas.toDataURL("image/jpeg", 0.92);
      const sliceHeightPt = thisSliceHeightPx / pxPerPt;
      if (!firstPage) pdf.addPage();
      pdf.addImage(sliceData, "JPEG", MARGIN_X, MARGIN_Y, imgWidthPt, sliceHeightPt);
