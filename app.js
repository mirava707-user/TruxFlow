...
function normalizeLoad(load) {
  if (load.stops && load.stops.length) {
    return { paidStatus: "unpaid", paidStubId: null, bolDataUri: "", bolFileName: "", bolType: "", ...load };
  }
  const stop = { id: uid(), receiverName: load.receiverName || "", city: load.receiverCity || "", state: load.receiverState || "", zip: load.receiverZip || "" };
  return { ...load, stops: [stop], paidStatus: load.paidStatus || "unpaid", paidStubId: load.paidStubId || null, bolDataUri: load.bolDataUri || "", bolFileName: load.bolFileName || "", bolType: load.bolType || "" };
}
...
function DispatchApp({ onSignOut, userEmail } = {}) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const keys = ["loads", "trucks", "drivers", "billTos", "shippers", "receivers", "tripExpenses", "payStubHistory", "settings", "iftaReports", "iftaRates", "iftaFavorites"];
        const results = await Promise.all(keys.map((k) => window.storage.get(k).catch(() => null)));
        const [t, tr, dr, bt, sh, rc, tp, hi, st, ifr, ifra, iff] = results;
  ...
  // ---- Loads ----
  function openNewLoad() { setLoadForm({ ...emptyLoad(), billTo: "Amazon" }); setEditingLoadId(null); setMilesStatus(""); setShowLoadForm(true); }
    ...
    <div>
      ...
      {showLoadForm && (
        <form onSubmit={saveLoad} className="load-entry-form">
          ...
          <div className="section-label">Bill of Lading (BOL)</div>
          {loadForm.bolDataUri ? (
          ...
          <button type="button" className="btn secondary" style={{ marginBottom: 16 }} onClick={() => bolInputRef.current && bolInputRef.current.click()}>
        ...
        {rows.map(({ trip: t, miles, gross, fin }) => (
          <div className="trip-card" key={t.id} onClick={() => openTrip(t)}>
            <div className="trip-card-top">
              <span className="trip-card-id"><span className="trip-number-pill">Trip {t.tripNumber}</span> <span className="trip-card-truck">· Truck {t.truck}</span></span>
              <span className="trip-card-meta trip-card-meta-bold">{shortDate(t.startDate)} – {shortDate(t.endDate)}</span>
            </div>
...
function FleetMenu({ setFleetView, trucks, drivers, billTos, shippers, receivers }) {
  const items = [
    ...
    { key: "ifta", icon: Calculator, name: "IFTA Calculator", count: null },
  ];
  return (
    <div>
      ...
      {items.map((it) => (
        <div className="fleet-menu-item" key={it.key} onClick={() => setFleetView(it.key)}>
          ...
          <div className="fleet-menu-text">
            ...
            <div className="count">{typeof it.count === "number" ? `${it.count} record${it.count === 1 ? "" : "s"}` : "Quarterly fuel tax reports"}</div>
          </div>
...
function SettingsTab({ setFleetView, settings, saveStartingNumber, nextLoadNumber, saveSettings, onSignOut, askConfirm }) {
  ...
  async function handleExportBackup() {
    ...
    try {
      const keys = ["loads", "trucks", "drivers", "billTos", "shippers", "receivers", "tripExpenses", "payStubHistory", "settings", "iftaReports", "iftaRates"];
