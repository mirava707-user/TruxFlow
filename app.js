...
function normalizeTrip(t) { return { paidStatus: "unpaid", paidStubId: null, driver2Color: "clear", ...t }; }
...
function emptyIftaReport() {
  const now = new Date();
  const q = Math.floor(now.getMonth() / 3) + 1;
  return { id: null, quarter: q, year: now.getFullYear(), truck: "ALL", rows: [], savedAt: null };
}
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const [t, tr, dr, bt, sh, rc, tp, hi, st, ifr, ifra] = results;
        ...
        {(() => {
          ...
          return (
            <>
              ...
              <div className="field-row">
                {renderField(byKey.otherCharges)}
                <div className="field">
                  <label>Other Charges Notes</label>
                  <input value={tripForm.otherNotes} onChange={(e) => setTripForm({ ...tripForm, otherNotes: e.target.value })} placeholder="Optional note" style={{ fontFamily: "Inter" }} />
                </div>
              </div>
            </>
          );
...
function IftaCalculatorPage(p) {
  const { setFleetView, truckNumbers, iftaReports, iftaRates, saveIftaRates, saveIftaReport, deleteIftaReport, askConfirm, companyInfo } = p;
  const [mode, setMode] = useState("build"); // build | history
  const [report, setReport] = useState(emptyIftaReport());
  ...
  function startNewReport() { setReport(emptyIftaReport()); setActiveReportId(null); setMode("build"); }
  function openReportForEdit(r) { setReport({ quarter: r.quarter, year: r.year, truck: r.truck, rows: r.rows }); setActiveReportId(r.id); setMode("build"); }

  function addRow() {
    if (!addJurisdiction) return;
    setReport((r) => ({ ...r, rows: [...r.rows, { jurisdiction: addJurisdiction, miles: "", gallons: "" }] }));
    setAddJurisdiction("");
  }
          ...
          <div className="stub-summary">
            <table>
              <tbody>
                ...
                <tr className="net-row"><td>{vt.netTotal >= 0 ? "Total Tax Due" : "Total Credit"}</td><td style={{ textAlign: "right" }}>{money(Math.abs(vt.netTotal))}</td></tr>
              </tbody>
            </table>
          </div>
        </div>
    ...
    <div>
            ...
            return (
              <div className="statement-card" key={r.id} onClick={() => setViewingReport(r)}>
                ...
                <div className="statement-card-row">
                  ...
                  <span className="statement-card-amt" style={{ color: rt.netTotal >= 0 ? "var(--red)" : "var(--green)" }}>
                    {rt.netTotal >= 0 ? `Owe ${money(rt.netTotal)}` : `Credit ${money(Math.abs(rt.netTotal))}`}
                  </span>
                </div>
              </div>
            );
      ...
      {mode === "build" && (
        <>
          ...
          <div className="field-row">
            <div className="field">
              <select value={addJurisdiction} onChange={(e) => setAddJurisdiction(e.target.value)}>
                <option value="">Select jurisdiction to add…</option>
                {availableJurisdictions.map((j) => <option key={j} value={j}>{j}</option>)}
              </select>
            </div>
            <button type="button" className="add-trip-btn" style={{ flexShrink: 0 }} onClick={addRow} disabled={!addJurisdiction}><Plus size={14} /> Add</button>
          </div>
          ...
          {report.rows.length === 0 ? (
            <div className="empty-state">No jurisdictions added yet — add one above or upload a sheet.</div>
          ) : (
            <div className="trip-card-list" style={{ marginBottom: 16 }}>
              {report.rows.map((row) => {
                const rate = num(iftaRates[row.jurisdiction] || 0);
                return (
                  <div className="trip-card" key={row.jurisdiction}>
                    <div className="trip-card-top">
                      <span className="trip-card-id">{row.jurisdiction} <span className="trip-card-truck">· ${rate.toFixed(4)}/gal</span></span>
                      <button type="button" className="mini-icon-btn" onClick={() => removeRow(row.jurisdiction)}><X size={14} /></button>
                    </div>
                    <div className="field-row" style={{ marginBottom: 0 }}>
                      <div className="field"><label>Miles</label><input type="number" value={row.miles} onChange={(e) => updateRow(row.jurisdiction, { miles: e.target.value })} placeholder="0" /></div>
                      <div className="field"><label>Gallons</label><input type="number" step="0.1" value={row.gallons} onChange={(e) => updateRow(row.jurisdiction, { gallons: e.target.value })} placeholder="0.0" /></div>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="pay-summary-box">
            ...
            <div className="pay-summary-row net">
              <span>{totals.netTotal >= 0 ? "Total Tax Due" : "Total Credit"}</span>
              <span style={{ color: totals.netTotal >= 0 ? "var(--red)" : "var(--green)" }}>{money(Math.abs(totals.netTotal))}</span>
            </div>
          </div>
