...
function DispatchApp({ onSignOut, userEmail } = {}) {
  ...
  const filteredLoads = useMemo(() => {
    ...
    if (q) list = list.filter((l) => norm(String(l.loadNumber || "")).includes(q) || norm(l.billTo).includes(q) || norm(l.driver).includes(q));
    return list;
  ...
  return (
    <div>
      ...
      <div className="search-box">
        ...
        <input value={loadSearch} onChange={(e) => setLoadSearch(e.target.value)} placeholder="Search by load #, bill to, or driver…" />
      </div>
...
function AuthGate() {
  ...
  useEffect(() => {
    if (session && session.user) {
      ...
      migrateLocalDataIfNeeded().finally(() => setMigrated(true));
    } else {
      currentUserId = null;
    }
  }, [session]);
