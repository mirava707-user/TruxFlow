...
function DashboardTab(p) {
  ...
  return (
    <div>
      ...
      {!isAnnualTax && <div style={isOregon ? { fontSize: 15, color: "var(--text)", fontWeight: 700, margin: "16px 0 10px" } : { fontSize: 11.5, color: "var(--text-dim)", margin: "16px 0 10px" }}>{fmtDate(dashReport.start)} – {fmtDate(dashReport.end)}</div>}
                  ...
                  return (
                    <div style={{ marginTop: 18, background: "var(--surface-2)", borderRadius: 12, padding: "4px 14px", marginBottom: 20 }}>
                      ...
                      <div style={{ padding: "10px 0" }}>
                        <div style={{ fontSize: 13, color: "var(--text)", marginBottom: 6 }}>Notes</div>
                        <input
                          key={`${orMonthKey}-notes`}
                          type="text"
                          defaultValue={orPay.notes || ""}
                          placeholder="Optional notes about this month's filing"
                          style={{ width: "100%", background: "#FFFFFF", color: "#14181F", border: "1px solid var(--border)", borderRadius: 8, padding: "9px 11px", fontSize: 13, fontFamily: "Inter, sans-serif" }}
