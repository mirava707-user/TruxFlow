...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .settings-btn { width: 48px; height: 48px; border-radius: 12px; color: var(--accent); flex-shrink: 0; margin-right: 2px; }
    ...
    <div>
      ...
      <div className="trip-table">
        <div className="trip-table-header">
          ...
          <span className="tt-col-truck tt-truck-header" style={{ position: "relative" }} onClick={() => setTruckFilterOpen((v) => !v)}>
            <span>{tripsTruckFilter === "ALL" ? "Truck" : tripsTruckFilter}</span>
            <ChevronDown size={9} className="tt-truck-arrow" />
            {truckFilterOpen && (
              <>
                <div className="color-picker-backdrop" onClick={(e) => { e.stopPropagation(); setTruckFilterOpen(false); }} />
                <div className="month-charge-popover" style={{ left: -4, right: "auto", minWidth: 120 }} onClick={(e) => e.stopPropagation()}>
                  <div className={`month-charge-item ${tripsTruckFilter === "ALL" ? "selected" : ""}`} onClick={() => { setTripsTruckFilter("ALL"); setTruckFilterOpen(false); }}>
                    <span>All Trucks</span>
                  </div>
                  {truckNumbers.map((tr) => (
                    <div key={tr} className={`month-charge-item ${tripsTruckFilter === tr ? "selected" : ""}`} onClick={() => { setTripsTruckFilter(tr); setTruckFilterOpen(false); }}>
                      <span>{tr}</span>
                    </div>
                  ))}
                </div>
          ...
          {dashReport.rows.map((r) => (
            <div className="dash-card" key={r.key}>
              <div className="dash-card-top">
                ...
                <span className="dash-card-badge dash-card-badge-link" onClick={() => viewGroupLoads(dashReport.groupBy, r.key, `${r.loads} load${r.loads === 1 ? "" : "s"} — ${r.key}`)}>{r.loads} load{r.loads === 1 ? "" : "s"}</span>
              </div>
          ...
          <div className="dash-card-list">
            ...
            {dashReport.rows.map((r) => (
              <div className="dash-card" key={r.key}>
                <div className="dash-card-top">
                  <span className="dash-card-name" title={r.key}>{r.key}</span>
                  <span className="dash-card-badge dash-card-badge-link" onClick={() => viewGroupLoads("driver", r.key, `${r.loads} load${r.loads === 1 ? "" : "s"} — ${r.key}`)}>{r.loads} load{r.loads === 1 ? "" : "s"}</span>
                </div>
                <div className="dash-card-body">
      ...
      <div className="dash-card-list">
        ...
        {dashReport.rows.map((r) => (
          <div className="dash-card" key={r.key}>
            <div className="dash-card-top">
              <span className="dash-card-name" title={r.key}>{r.key}</span>
              <span className="dash-card-badge dash-card-badge-link" onClick={() => viewGroupLoads(dashReport.viewBy === "billto" ? "billTo" : dashReport.viewBy, r.key, `${r.loads} load${r.loads === 1 ? "" : "s"} — ${r.key}`)}>{r.loads} load{r.loads === 1 ? "" : "s"}</span>
            </div>
            <div className="dash-card-body">
              <div className="dash-card-hero"><span className="lbl">Gross</span><span className="val">{compactMoney(r.gross)}</span></div>
            </div>
            ...
            <table>
              <tbody>
                ...
                {(r.cancellationItems || []).map((item, i) => (
                  <tr className="refund-row" key={`cx${i}`}><td>Cancellation — {item.note}</td><td style={{ textAlign: "right" }}>+{money(item.amount)}</td></tr>
                ))}
  ...
  return (
    <div>
      <div className="section-label-row" style={{ marginTop: 0 }}>
        ...
        <button className="icon-btn settings-btn" onClick={() => setFleetView("settings")} title="Settings"><SettingsIcon size={24} /></button>
      </div>
      {items.map((it) => (
