...
function TrendChart({ data, labels, color, formatY }) {
  const width = 600, height = 220, padL = 44, padB = 22, padT = 14, padR = 10;
    ...
    <div className={`theme-${settings.theme || "dark"}`} style={{ background: "var(--bg)", minHeight: "100vh", fontFamily: "Inter, sans-serif" }}>
      ...
      <div className="app-shell">
        ...
        <div className={`content ${textScale === "large" ? "text-large" : ""}`}>
          ...
          {tab === "stats" && (
            <StatsTab {...{ loads, trips, computeTripProfit, settings }} />
          )}
...
function StatsTab({ loads, trips, computeTripProfit, settings }) {
  ...
  const { start, end, prevStart, prevEnd } = useMemo(() => {
    ...
    if (period === "month") return { start: addDays(today, -29), end: today, prevStart: addDays(today, -59), prevEnd: addDays(today, -30) };
  ...
  function statsForRange(s, e) {
    const rangeTrips = trips.filter((t) => t.startDate && t.startDate >= s && t.startDate <= e);
    ...
    const totalExpenses = rangeTrips.reduce((sum, t) => sum + EXPENSE_REPORT_FIELDS.reduce((s2, f) => s2 + num(t[f.key]), 0), 0);
    const companyProfit = grossRevenue - totalExpenses;
    ...
    return { grossRevenue, totalMiles, totalExpenses, companyProfit, totalLoadsCount, avgRPM, profitMargin, rows };
  }
  ...
  const dailySeries = useMemo(() => {
    ...
    let d = start;
    let guard = 0;
    while (d <= end && guard < 400) { days.push(d); d = addDays(d, 1); guard++; }
    return days.map((day) => {
      const dayTrips = trips.filter((t) => t.startDate === day);
      const dayRows = dayTrips.map((t) => computeTripProfit(t));
      const gross = dayRows.reduce((s, r) => s + r.gross, 0);
      const miles = dayRows.reduce((s, r) => s + r.miles, 0);
      const dayExpenses = dayTrips.reduce((s, t) => s + EXPENSE_REPORT_FIELDS.reduce((s2, f) => s2 + num(t[f.key]), 0), 0);
      const dayLoadsCount = loads.filter((l) => l.status === "completed" && (l.deliveryDate || l.pickupDate) === day).length;
      return { day, gross, miles, loads: dayLoadsCount, profit: gross - dayExpenses };
    });
  ...
  const topDrivers = useMemo(() => {
    ...
    current.rows.forEach((r) => {
      ...
      byDriver[name].loads += (r.trip.loadIds || []).length;
    });
  ...
  const chartLabels = dailySeries.map((d) => shortDate(d.day));
  ...
  const statCards = [
    ...
    { label: "Profit Margin", value: current.profitMargin.toFixed(1) + "%", delta: pctChange(current.profitMargin, previous.profitMargin), series: dailySeries.map((d) => (d.gross > 0 ? ((d.gross - (d.gross - d.profit)) / d.gross) * 100 : 0)), color: "var(--green)" },
  ];
    ...
    <div className="stats-page">
      ...
      {period === "custom" && (
        <div className="field-row" style={{ marginBottom: 14 }}>
          <div className="field"><label>From</label><input type="date" value={customStart} max={customEnd} onChange={(e) => setCustomStart(e.target.value)} /></div>
          <div className="field"><label>To</label><input type="date" value={customEnd} min={customStart} max={todayISO()} onChange={(e) => setCustomEnd(e.target.value)} /></div>
        </div>
      )}
      ...
      <div className="stats-main-row">
        ...
        <div className="stats-panel stats-panel-profit">
          ...
          <div className="stats-profit-row"><span>Total Expenses</span><span className="stats-profit-val" style={{ color: "var(--red)" }}>{money(current.totalExpenses)}</span></div>
