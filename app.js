...
function DispatchApp({ onSignOut, userEmail } = {}) {
  ...
  function computeDriverPay(driverName, start, end) {
    ...
    return { driver, loadPays, grossPay, isMileage, loadedMilesPay, emptyMilesPay, dispatchFeePercent, dispatchFee, expenseBreakdown, expenseTotal, refunds, netPay, trucksUsed, tripIds, otherChargeItems, insuranceMonths, logbookMonths, orPermitNotes, truckPayNotes, refundsNotes };
  }
      ...
      {!isAnnualTax && (dashViewBy === "dispatcher" ? (
        ...
        <>
          <div className="field-row">
            <div className="field" style={{ position: "relative" }}>
              <label>Drivers</label>
              <button type="button" className="month-select-btn" onClick={() => setDriverDropdownOpen((v) => !v)}>
                <span>{dashDriverFilter.length === 0 ? "All Drivers" : `${dashDriverFilter.length} selected`}</span>
                <ChevronDown size={14} />
              </button>
              {driverDropdownOpen && (
                <>
                  ...
                  <div className="month-charge-popover">
                    ...
                    <button type="button" className="btn" style={{ marginTop: 6 }} onClick={() => setDriverDropdownOpen(false)}>Done</button>
                  </div>
                </>
              )}
            </div>
          ...
          <div className="stub-summary">
            <table>
              <tbody>
                ...
                {r.refunds > 0 && <tr className="refund-row"><td>Refunds{r.refundsNotes && r.refundsNotes.length > 0 ? ` (${r.refundsNotes.join("; ")})` : ""}</td><td style={{ textAlign: "right" }}>+{money(r.refunds)}</td></tr>}
                ...
                <div className="stub-summary">
                  <table>
                    <tbody>
                      ...
                      {stub.refunds > 0 && <tr className="refund-row"><td>Refunds{stub.refundsNotes && stub.refundsNotes.length > 0 ? ` (${stub.refundsNotes.join("; ")})` : ""}</td><td style={{ textAlign: "right" }}>+{money(stub.refunds)}</td></tr>}
