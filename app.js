...
async function generatePdf(filename) {
  ...
  try {
    ...
    const canvas = await window.html2canvas(clone, { scale: 2, backgroundColor: "#ffffff", useCORS: true, width: PDF_WIDTH, windowWidth: PDF_WIDTH });
...
function normalizeTrip(t) {
  const base = { paidStatus: "unpaid", paidStubId: null, driver2Color: "clear", otherChargesList: [], logbookMonth: "", insuranceMonth: "", orPermitNote: "", truckPayNote: "", tripStatus: "active", tripNote: "", tripNoteColor: "clear", refundsNote: "", ...t };
...
function DispatchApp({ onSignOut, userEmail } = {}) {
  ...
  const filteredLoads = useMemo(() => {
    if (tripLoadsFilter) {
      return loads
        .filter((l) => l.truck === tripLoadsFilter.truck && l.status === "completed" && inRange(l.deliveryDate || l.pickupDate, tripLoadsFilter.start, tripLoadsFilter.end))
        ...
        <div className={`content ${textScale === "large" ? "text-large" : ""}`}>
          {tab === "loads" && (
            <LoadsTab {...{
              ...
              companyInfo: settings, closeAllActiveLoads, askConfirm, tripLoadsFilter, setTripLoadsFilter,
            }} />
          )}

          {tab === "expenses" && expensesView === "list" && (
            <ExpensesList {...{ trips, computeTripProfit, computeTripFinancials, openTrip, openNewTrip, openNewPendingTrip, tripsMonth, setTripsMonth, tripsYear, setTripsYear, settings }} />
          )}
          {tab === "expenses" && expensesView === "detail" && (
            <ExpenseDetail {...{
              ...
              driverByName, askConfirm, nextTripNumber, settings, setTab, setTripLoadsFilter,
            }} />
          )}
...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .trip-table-gross { font-weight: 800; font-size: 17px; }
  @media (min-width: 900px) {
    .tt-col-fee { flex: 1.4; }
  ...
  .pdf-mode.stub-sheet { border-radius: 0; padding: 20px 16px; }
...
function LoadsTab(p) {
  const {
    ...
    filterStatus, setFilterStatus, filterTruck, setFilterTruck, loadSearch, setLoadSearch,
    filteredLoads, expandedLoadId, setExpandedLoadId, editLoad, closeLoad, reopenLoad, deleteLoad, duplicateLoad, loadsQuickStats,
    companyInfo, closeAllActiveLoads, askConfirm, tripLoadsFilter, setTripLoadsFilter,
  } = p;
    ...
    <div>
      ...
      {tripLoadsFilter ? (
        <div className="trip-loads-banner">
          <span>Showing loads for <strong>{tripLoadsFilter.tripLabel}</strong> · Truck {tripLoadsFilter.truck}</span>
            ...
            <div className="card-head" onClick={() => setExpandedLoadId(open ? null : l.id)}>
              <div className="load-card-row1">
                <div className="load-card-row1-left">
                  ...
                  <span className="load-dates-inline">{shortDate(l.pickupDate)}</span>
                </div>
...
function ExpensesList({ trips, computeTripProfit, computeTripFinancials, openTrip, openNewTrip, openNewPendingTrip, tripsMonth, setTripsMonth, tripsYear, setTripsYear, settings }) {
    ...
    <div>
      ...
      <div className="trip-table">
        <div className="trip-table-header">
          ...
          <span className="trip-table-dot" style={{ opacity: 0 }}>·</span>
          <span className="tt-col-fee">Profit</span>
          <span className="trip-table-dot" style={{ opacity: 0 }}>·</span>
        ...
        {rows.map(({ trip: t, miles, gross, fin }) => (
          ...
          >
            <span className="tt-col-trip trip-table-num" style={t.tripStatus === "pending" ? { color: "#E15C4F", fontWeight: 900, fontSize: 18 } : undefined}>{t.tripStatus === "pending" ? "P" : t.tripNumber}</span>
            ...
            <span className="trip-table-dot">·</span>
            <span className="tt-col-gross trip-table-gross" style={{ color: t.paidStatus === "paid" ? "#4A90D9" : "var(--text)" }}>{money(gross)}</span>
          </div>
        ))}
      </div>
...
function ExpenseDetail(p) {
  const { tripForm, setTripForm, activeTripId, truckNumbers, driverNames, computeTripMilesGross, computeTripProfit, saveTrip, deleteTrip, closeTripDetail, driverByName, askConfirm, nextTripNumber, settings, setTab, setTripLoadsFilter } = p;
            ...
            <>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setTripLoadsFilter({ truck: tripForm.truck, start: tripForm.startDate, end: tripForm.endDate, tripLabel: label });
                  setTab("loads");
                }}
              >
...
function DashboardTab(p) {
  ...
  return (
    <div>
      <div className="field-row">
        <div className="field">
          ...
          <select value={dashViewBy} onChange={(e) => setDashViewBy(e.target.value)}>
            ...
            <option value="oregon">Oregon Permit</option>
            <option value="annualTax">Annual Tax Report</option>
          </select>
        </div>
      </div>
      ...
      {isAnnualTax && (
        <div className="field-row">
          <div className="field">
            ...
            <select value={annualTaxYear} onChange={(e) => setAnnualTaxYear(Number(e.target.value))}>
              {(() => { const y = new Date().getFullYear(); const arr = []; for (let i = y; i >= y - 6; i--) arr.push(i); return arr; })().map((y) => <option key={y} value={y}>{y}</option>)}
            </select>
          </div>
        </div>
      )}
      {!isAnnualTax && dashPeriod === "custom" && (
        <div className="field-row">
      ...
      {!isAnnualTax && <button className="btn" onClick={runDashUpdate}>Update</button>}
          ...
          {annualTaxReport.rows.length > 0 && (
            <>
              <table className="stub-loads-table">
                <thead>
                  <tr><th>#</th><th>Driver</th><th>{"Loads/Miles"}</th><th style={{ textAlign: "right" }}>Annual Income</th><th></th></tr>
                </thead>
                <tbody>
      ...
      {!isProfit && !isExpenses && !isMiles && !isDispatcher && !isOregon && !isAnnualTax && (
      <div style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 10, lineHeight: 1.5 }}>
        ...
        <div className="modal-overlay" onClick={() => setPrintingTaxDriver(null)}>
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
            <div className="print-area stub-sheet">
              ...
              <table className="stub-table">
                <tbody>
                  {printingTaxDriver.isMileage && <tr><td style={{ color: "#666" }}>Total Miles Driven</td><td style={{ textAlign: "right" }}>{printingTaxDriver.totalMiles.toLocaleString()} mi</td></tr>}
                  {printingTaxDriver.isMileage && <tr><td style={{ color: "#666" }}>Rate</td><td style={{ textAlign: "right" }}>${num(printingTaxDriver.driver.rate).toFixed(2)}/mi</td></tr>}
                  <tr><td style={{ color: "#666" }}>Total Loads</td><td style={{ textAlign: "right" }}>{printingTaxDriver.loadCount}</td></tr>
                  {printingTaxDriver.trucksUsed.length > 0 && <tr><td style={{ color: "#666" }}>Truck(s)</td><td style={{ textAlign: "right" }}>{printingTaxDriver.trucksUsed.join(", ")}</td></tr>}
                </tbody>
              </table>
        ...
        <div className="modal-overlay" onClick={() => setPrintingTaxAll(false)}>
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
            <div className="print-area stub-sheet">
              ...
              <table className="stub-loads-table">
                <thead><tr><th>#</th><th>Driver</th><th>Loads/Miles</th><th style={{ textAlign: "right" }}>Annual Income</th></tr></thead>
                <tbody>
                  {annualTaxReport.rows.map((r, i) => (
                    <tr key={r.driver.id}>
                      <td>{i + 1}</td>
            ...
            <>
              ...
              {unpaidLoads.length > 0 && (
                <>
                  <table className="stub-loads-table">
                    <thead>
                      <tr><th>#</th><th>Load#</th><th>WO#</th><th>Date</th><th style={{ textAlign: "right" }}>Rate</th></tr>
                    </thead>
                    <tbody>
                      {unpaidLoads.map((l, i) => (
                        <tr key={l.id}>
    ...
    return (
      <div>
        ...
        <div className="print-area stub-sheet">
          ...
          <table className="stub-table">
            <thead><tr><th>Load #</th><th>Route</th><th style={{ textAlign: "right" }}>Pay</th></tr></thead>
            <tbody>
              {(!r.loadSnapshots || r.loadSnapshots.length === 0) && <tr><td colSpan={3} style={{ color: "#999" }}>Salary pay period — not tied to individual loads.</td></tr>}
              {(r.loadSnapshots || []).map((l) => (
                <tr key={l.loadId}>
                  <td>{l.loadNumber}</td>
                  <td style={{ fontSize: 10, lineHeight: 1.4 }}>{routeFullJSX(l)}</td>
            ...
            <>
              ...
              {unpaidLoads.length > 0 && (
                <>
                  <table className="stub-loads-table">
                    <thead>
                      <tr><th>#</th><th>Load#</th><th>Bill To</th><th>Date</th><th style={{ textAlign: "right" }}>Rate</th></tr>
                    </thead>
                    <tbody>
                      {unpaidLoads.map((l, i) => (
                        <tr key={l.id}>
            ...
            <>
              <div className="print-area stub-sheet" style={{ marginTop: 16 }}>
                ...
                <table className="stub-table">
                  <thead><tr><th>Load #</th><th>Route</th><th style={{ textAlign: "right" }}>Pay</th></tr></thead>
                  <tbody>
                    {stub.loadPays.length === 0 && <tr><td colSpan={3} style={{ color: "#999" }}>No unpaid completed loads in this period.</td></tr>}
                    {stub.loadPays.map((l) => (
                      <tr key={l.id}>
                        <td>{l.loadNumber}</td>
                        <td style={{ fontSize: 10, lineHeight: 1.4 }}>{routeFullJSX(l)}</td>
...
function AutocompleteInput({ value, onChange, onSelect, options, placeholder, getLabel, getSub }) {
  ...
  const filtered = (q && typed
    ? options.filter((o) => norm(getLabel(o)).includes(q) || (getSub && norm(getSub(o) || "").includes(q)))
    : options
  ).slice(0, 6);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const isMatch = (o) => norm(getLabel(o)).startsWith(q) || (getSub && norm(getSub(o) || "").startsWith(q));
      const best = options.find(isMatch);
      if (best) { onSelect(best); setTyped(false); }
    }, 2000);
    ...
    <div style={{ position: "relative" }}>
      ...
      {focused && filtered.length > 0 && (
        <div className="autocomplete-dropdown">
          {filtered.map((o, i) => (
            <div className="autocomplete-item" key={i} onMouseDown={() => { onSelect(o); setTyped(false); }}>
