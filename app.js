...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .stub-load-card { background: var(--surface); border: 1px solid var(--border); border-radius: 10px; padding: 10px 12px; margin-bottom: 8px; }
  .stub-load-card-top { display: flex; align-items: center; gap: 8px; margin-bottom: 8px; }
  .stub-load-num { font-family: 'Oswald', sans-serif; font-weight: 700; font-size: 13.5px; color: var(--text); }
  .stub-load-card-bottom { display: flex; justify-content: space-between; align-items: center; }
  .stub-load-rate { font-family: 'IBM Plex Mono', monospace; font-weight: 700; font-size: 14px; color: var(--accent); }
          ...
          <div className="trip-card" key={t.id} onClick={() => openTrip(t)}>
            ...
            <div className="dash-card-body trip-stats-row">
              ...
              <div className="trip-fee-gross-group">
                ...
                {t.paidStatus === "paid" && <span className="trip-paid-flag">✓ PAID</span>}
    ...
    if (!showFullPdf) {
      return (
        <div>
          ...
          {(!r.loadSnapshots || r.loadSnapshots.length === 0) && <div className="empty-state">Salary pay period — not tied to individual loads.</div>}
          {(r.loadSnapshots || []).map((l) => {
            const live = loadsById[l.loadId];
            const paidState = live ? (live.paidStatus || "unpaid") : "paid";
            return (
              <div className="stub-load-card" key={l.loadId}>
                <div className="stub-load-card-top">
                  <span className="stub-load-num">#{l.loadNumber}</span>
                  {live && live.workOrder && <span className="truck-standout">{live.workOrder}</span>}
                  <span className="load-dates-inline">{live ? mmdd(live.deliveryDate || live.pickupDate) : "—"}</span>
                </div>
                <div className="stub-load-card-bottom">
                  <span className={`paid-pill ${paidState}`}>{paidState}</span>
                  <span className="stub-load-rate">{money(live ? live.rate : l.driverPay)}</span>
                </div>
              </div>
            );
          })}
          ...
          {relatedTrips.length > 0 && (
            <>
              <div className="section-label" style={{ marginTop: 22 }}>Related Trips</div>
              {relatedTrips.map((t) => (
                <div className="history-item" key={t.id} style={{ cursor: "default" }}>
                  <span>Trip {t.tripNumber} · Truck {t.truck}</span>
                  <span style={{ fontSize: 11, color: "var(--text-dim)", fontFamily: "IBM Plex Mono, monospace" }}>{shortDate(t.startDate)} – {shortDate(t.endDate)}</span>
                </div>
              ))}
            </>
          )}
