        ...
        <div className={`content ${textScale === "large" ? "text-large" : ""}`}>
          ...
          {tab === "expenses" && expensesView === "detail" && (
            <ExpenseDetail {...{
              ...
              driverByName, askConfirm, nextTripNumber, settings,
            }} />
          )}
...
function ExpenseDetail(p) {
  const { tripForm, setTripForm, activeTripId, truckNumbers, driverNames, computeTripMilesGross, computeTripProfit, saveTrip, deleteTrip, closeTripDetail, driverByName, askConfirm, nextTripNumber, settings } = p;
...
function AutocompleteInput({ value, onChange, onSelect, options, placeholder, getLabel, getSub }) {
  ...
  const q = norm(value);
  const filtered = (q && typed ? options.filter((o) => norm(getLabel(o)).includes(q)) : options).slice(0, 6);
  return (
    <div style={{ position: "relative" }}>
      <input
        type="text"
