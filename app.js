  ...
  * { box-sizing: border-box; }
  ...
  @media (min-width: 900px) {
    ...
    .text-scale-btn {
      ...
      font-family: 'Oswald', sans-serif; font-size: 13px; font-weight: 700; cursor: pointer;
    }
    .text-scale-btn:hover { border-color: var(--accent); color: var(--accent); }
...
function LoadsTab(p) {
  ...
  return (
    <div>
      ...
      {!showLoadForm && (
        <div className="report-grid" style={{ marginBottom: 18, gridTemplateColumns: "1fr 1fr" }}>
          <div className="stat-box">
            <div className="num">{compactMoney(loadsQuickStats.weekGross)}</div>
            <div className="lbl">This Week's Gross</div>
          </div>
          <div className="stat-box">
            <div className="num" style={{ color: "var(--accent)" }}>{compactMoney(loadsQuickStats.activeValue)}</div>
            <div className="lbl">Active Load Value ({loadsQuickStats.activeCount})</div>
          </div>
        </div>
      )}
          ...
          <div className="card" key={l.id}>
            <div className="card-head" onClick={() => setExpandedLoadId(open ? null : l.id)}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 2 }}>
                {l.workOrder && <span style={{ fontSize: 10, fontWeight: 400, color: "var(--text-dim)" }}>WO {l.workOrder}</span>}
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
