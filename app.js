...
// ---- Account modal: My Profile + Manage Users (team invite/roster) ----
...
function AccountModal({ userEmail, onSignOut, onClose }) {
  const hasBackend = typeof sb !== "undefined";
  const [tab, setTab] = useState("profile"); // "profile" | "manage"
  ...
  useEffect(() => {
    if (tab === "manage" && roster === null) loadRoster();
      ...
      <div className="account-modal-box" onClick={(e) => e.stopPropagation()}>
        <div className="account-modal-header">
          <div className="account-modal-tabs">
            <button type="button" className={`account-modal-tab ${tab === "profile" ? "active" : ""}`} onClick={() => setTab("profile")}>My Profile</button>
            <button type="button" className={`account-modal-tab ${tab === "manage" ? "active" : ""}`} onClick={() => setTab("manage")}>Manage Users</button>
          </div>
        ...
        {tab === "profile" && (
          <div className="account-modal-body">
            ...
            <button type="button" className="btn danger" style={{ marginTop: 14, width: "100%" }} onClick={() => { onClose(); onSignOut(); }}>Sign Out</button>
          </div>
        )}
                ...
                {isOwner ? (
                  <div className="account-modal-invite-section">
                    ...
                    <div style={{ fontSize: 12, color: "var(--text-dim)", marginBottom: 10, lineHeight: 1.5 }}>
                      Generate a code and share it with them yourself (text, email, however). They enter it as an "Invite Code" when creating their account, and they'll see and edit the same data you do — trucks, loads, everything.
                    </div>
                    {inviteCode ? (
                      ...
                      <button type="button" className="btn" onClick={handleGenerateInvite} disabled={generating}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div style={{ position: "relative" }}>
                ...
                {accountModalOpen && onSignOut && (
                  <AccountModal userEmail={userEmail} onSignOut={onSignOut} onClose={() => setAccountModalOpen(false)} />
                )}
              </div>
            </div>
          </div>
...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .account-modal-box { width: 100%; max-width: 420px; background: var(--surface); border-radius: 16px 16px 0 0; box-shadow: 0 -8px 30px rgba(0,0,0,0.4); max-height: 85vh; overflow-y: auto; }
  .account-modal-header { display: flex; align-items: center; justify-content: space-between; padding: 14px 14px 0; }
  .account-modal-tabs { display: flex; gap: 6px; }
  .account-modal-tab { background: var(--surface-2); border: 1px solid var(--border); color: var(--text-dim); font-family: 'Oswald', sans-serif; font-weight: 700; font-size: 12.5px; text-transform: uppercase; letter-spacing: 0.3px; padding: 9px 14px; border-radius: 10px; cursor: pointer; }
  .account-modal-tab.active { background: var(--accent); color: #1A1300; border-color: var(--accent); }
