        ...
        <>
          ...
          <div className="loads-status-row">
            <button type="button" className={`loads-status-seg ${filterStatus === "active" ? "active" : ""}`} onClick={() => setFilterStatus("active")}>
              <span className="loads-status-dot" style={filterStatus === "active" ? { background: "#1A1300" } : undefined} /> Active <span className="loads-status-count">{activeCount}</span>
            </button>
            <button type="button" className={`loads-status-seg ${filterStatus === "completed" ? "active" : ""}`} onClick={() => setFilterStatus("completed")}>
              <CheckCircle2 size={14} color={filterStatus === "completed" ? "#1A1300" : "var(--green)"} /> Completed <span className={`loads-status-count ${filterStatus === "completed" ? "" : "loads-status-count-green"}`}>{completedCount}</span>
            </button>
