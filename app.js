...
function AccountSidebar({ userEmail, onSignOut, onClose, settings, saveSettings, saveStartingNumber, nextLoadNumber, askConfirm, hasUnreadSupport }) {
  const hasBackend = typeof sb !== "undefined";
        ...
        {view === "menu" && (
          <div className="account-sidebar-menu">
            ...
            <button type="button" className="account-sidebar-menu-item" onClick={() => setView("help")}>
              <MessageCircle size={17} /> <span>Help & Support</span> {hasUnreadSupport && <span className="account-sidebar-menu-item-dot" />} <ChevronRight size={16} className="account-sidebar-menu-arrow" />
            </button>
            <div style={{ height: 28 }} />
            <button type="button" className="account-sidebar-menu-item account-sidebar-signout" onClick={() => { cachedTeamRoster = null; onClose(); onSignOut(); }}>
...
function DispatchApp({ onSignOut, userEmail } = {}) {
  ...
  async function checkUnreadSupport() {
    ...
    try {
      const { data } = await sb.rpc("has_unread_support_messages");
      setHasUnreadSupport(!!data);
    } catch (e) {
          ...
          <div className="header-row">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div style={{ position: "relative" }}>
                ...
                >
                  <User size={16} />
                  {hasUnreadSupport && <span className="unread-dot" />}
                </button>
...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .account-sidebar-menu-item-dot { width: 8px; height: 8px; border-radius: 50%; background: var(--red); flex-shrink: 0; margin-left: -6px; }
  ...
  .account-sidebar-menu { display: flex; flex-direction: column; flex: 1; min-height: 0; }
  ...
  .theme-light .row-line { border-bottom-color: #5A6270; }
  ...
  return (
    <div>
      ...
      {showLoadForm && (
        <form onSubmit={saveLoad} className="load-entry-form">
          ...
          <div className="dash-filter-grid" style={{ marginBottom: 12 }}>
            <FilterCard
              icon={User} label="Driver" value={loadForm.driver} onChange={(v) => setLoadForm({ ...loadForm, driver: v })}
              options={driverNames.map((d) => ({ value: d, label: d }))}
            />
            <FilterCard
              icon={Truck} label="Truck" value={loadForm.truck} onChange={(v) => setLoadForm({ ...loadForm, truck: v })}
      ...
      {[...filteredLoads].sort((a, b) => (b.loadNumber || 0) - (a.loadNumber || 0)).map((l, i) => {
        ...
        const cardStyle = { ...(i % 2 === 1 ? { background: "var(--surface-2)" } : null), ...(l.notes ? { borderRight: "3px solid #E15C4F" } : null) };
        return (
              ...
              <table className="stub-table">
                ...
                <tbody>
                  ...
                  {printingLoad.shipperNotes && (
                    <tr><td colSpan={4} style={{ color: "#4D4D4D", fontStyle: "italic" }}>Note: {printingLoad.shipperNotes}</td></tr>
                  )}
                  {(printingLoad.stops || []).map((s, i) => (
                    <Fragment key={s.id || i}>
                      ...
                      {s.notes && (
                        <tr><td colSpan={4} style={{ color: "#4D4D4D", fontStyle: "italic" }}>Note: {s.notes}</td></tr>
                      )}
                    </Fragment>
                  ))}
                </tbody>
